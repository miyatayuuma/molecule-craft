...
export async function simulateChoCampaign({seed=1,propellant='hydrogen',fps=60}={}){
  ...
  const resources=createResources({storage});resources.setCatalog(catalog);const sorties=[],crafts=[];let model=null;
  function craft(id){
    if(model?.id===id)return;
    if(model){resources.refund(model.cost);model=null;} // dismantle the previous paid model
    ...
    resources.discover(id);model={id,cost};resources.save();if(!crafts.includes(id))crafts.push(id);
  }
  function fill(use,id){
    craft(id); // each charge is backed by the completed handmade model
    const plan=resources.tankFillPlan(use,id);if(plan?.maxAdd)resources.fillTankFromElements(use,id,plan.maxAdd);
  }
  function sortie({name,start='veil',routes=[],seconds=65,burn=false,stop=()=>false,final=false}){
    ...
    sorties.push({name,seconds:+run.time.toFixed(2),returned,captured:run.captured,destinationReached:run.destinationReached,result,stock:{...resources.state.elements},remaining:structuredClone(resources.state.tanks)});
  ...
  craft('hydrogen');fill('propellant','hydrogen');
  ...
  fill('fuel','methane');fill('oxidizer','oxygen');fill('coolant','water');fill('propellant',propellant);
  const burnSeconds=Math.min(resources.state.tanks.fuel.amount,resources.state.tanks.oxidizer.amount/2)*2;
