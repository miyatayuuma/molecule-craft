...
await update('scripts/build-molecule-db.mjs',source=>{
  ...
  for(const [start,end] of [
    ['for(const [id,nameJa,nameEn,count,iupacNameEn,learningNote] of [',']) add({id,nameJa,nameEn,iupacNameEn,learningNote,...diacid(count),category:\'dicarboxylic-acid\'});'],
    ['for(const [id,nameJa,nameEn,side,iupacNameEn,learningNote] of [',']){\n  const graph=acid(3);'],
  ]){
    const a=text.indexOf(start);if(a<0)throw new Error(`Missing tuple start ${start}`);const b=text.indexOf(end,a);if(b<0)throw new Error(`Missing tuple end ${end}`);
    const block=text.slice(a,b).replace(/,(?:'[^'\n]*'|"[^"\n]*")(\],?)$/gm,'$1');text=text.slice(0,a)+block+text.slice(b);
  }
  ...
  text=text.replace(']) add({id,nameJa,nameEn,iupacNameEn,learningNote,...diacid(count),category:\'dicarboxylic-acid\'});',']) add({id,nameJa,nameEn,iupacNameEn,...diacid(count),category:\'dicarboxylic-acid\'});');
  ...
  text=text.replace('add({id,nameJa,nameEn,iupacNameEn,learningNote,stereochemistry:\'unspecified\'','add({id,nameJa,nameEn,iupacNameEn,stereochemistry:\'unspecified\'');
  text=text.replace(/,learningNote\s*:\s*(?:'[^'\n]*'|"[^"\n]*")/g,'');
  if(text.includes('learningNote'))throw new Error('learningNote remains in generator');
  return text;
});
