...
const graphUrl=new URL('data/molecule-graph.json',root);
const outputUrl=new URL('docs/maps/molecule-graph.svg',root);
...
const nodeClass=node=>{const roles=new Set(roleNames(node));return roles.has('ROOT')?'root':roles.has('HUB')?'hub':roles.has('BRIDGE')?'bridge':roles.has('LEAF')?'leaf':'branch';};
...
const nodeGroups=nodes.map(node=>{
  ...
  const r=klass==='root'?17:klass==='hub'?14:10;
...
const sectorLabels=Object.entries(sectorAngle).map(([sector,deg])=>{const angle=deg*Math.PI/180,radius=1160;return `<text class="sector" x="${(cx+Math.cos(angle)*radius).toFixed(1)}" y="${(cy+Math.sin(angle)*radius).toFixed(1)}">${sector}</text>`;}).join('\n');
const svg=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="title desc">
<title id="title">Molecule Graph developer map</title>
<desc id="desc">Developer-only overview of ${nodes.length} production molecule nodes and ${edges.length} undirected structural-neighbor edges.</desc>
<style>
:root{color-scheme:dark}.bg{fill:#081018}.ring{fill:none;stroke:#26394a;stroke-width:1;stroke-dasharray:7 11}.axis{stroke:#31495d;stroke-width:1}.edge{stroke:#60778a;stroke-width:1.2;stroke-opacity:.48}.node circle{fill:#25394a;stroke:#9bb4c8;stroke-width:1.4}.node text{fill:#d9e6ef;font:10px ui-monospace,SFMono-Regular,Menlo,monospace;text-anchor:middle}.node.root circle{fill:#245468;stroke:#c6f1ff;stroke-width:3}.node.hub circle{fill:#5a4530;stroke:#ffd59c;stroke-width:2.5}.node.bridge circle{fill:#3c465f;stroke:#b9c8ff;stroke-width:2}.node.leaf circle{fill:#29343d;stroke:#7d919e}.sector{fill:#7890a3;font:700 18px ui-monospace,SFMono-Regular,Menlo,monospace;text-anchor:middle}.meta{fill:#cfe0ea;font:16px ui-monospace,SFMono-Regular,Menlo,monospace}.legend{fill:#9db1bf;font:13px ui-monospace,SFMono-Regular,Menlo,monospace}
</style>
...
<g transform="translate(42 52)"><text class="meta">Molecule Graph · ${nodes.length} nodes · ${edges.length} edges</text><text class="legend" y="24">ROOT cyan · HUB amber · BRIDGE blue · LEAF gray · radial distance ≈ graph depth · sector = weak family bias</text><text class="legend" y="44">Production graph source of truth for encyclopedia discovery relationships.</text></g>
</svg>\n`;
if(process.argv.includes('--check')){
  let current='';try{current=await readFile(outputUrl,'utf8');}catch{}
  if(current!==svg){console.error('molecule-graph.svg is stale. Run node scripts/export-molecule-graph.mjs');process.exit(1);}
  ...
  console.log(`Wrote docs/maps/molecule-graph.svg (${nodes.length} nodes / ${edges.length} edges).`);
}
