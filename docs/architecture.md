...
RL-1Rではreaction candidateはrule/species/instance/site IDだけを運び、viewerがinstance IDから3D objectを解決します。分子移動はcamera-facing planeのray intersectionで行います。RL-1Eではatomごとのbounded / neutralized interaction chargeをfunctional-group contextから導出し、formal chargeとoptional net ionic chargeから分離します。`decomposeMoleculePairInteraction()`がproductionとtestで同一のatom-pair Coulomb、visual-radius-based excluded volume、donor-H···acceptor spring、molecule force / torque authorityを提供し、debug用途ではpairwise force termsも返します。Hydrogen bondはdonor-Hからacceptor atomへのstateful springで、hysteresisに加えdonor-H、acceptor、molecule-pair occupancyを制限します。`reaction-lab-browser.test.mjs` は390×844 browser上でcanvas drag reaction、H-bond follow/break、およびwater/water、water/CO₂、carbonic-acid/CO₂のforce sign / steric probesを検証します。
