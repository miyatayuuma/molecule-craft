...
q('open-supply').click();assert.equal(q('supply-dialog').open,true);assert.equal(q('open-supply').textContent.trim(),'収集殻');assert.equal(q('shell-propellant').querySelector('small').textContent,'H₂ 120/120');assert.equal(q('tank-model-name').textContent,'H₂ · 水素');assert.equal(q('tank-comparison').children.length,2);
...
q('open-supply').click();q('launch-veil').click();assert.equal(q('craft-target').hidden,true,'Launching exploration clears the craft target');const fuel=game.run('resources.state.tanks.propellant.amount');assert.equal(q('veil-fuel').textContent,'3 / 3');assert.match(q('veil-burst-meter').style.transform,/scaleX\([\d.]+\)/);
...
assert.equal(q('veil-fuel').textContent,'2 / 3','BURST gauge follows the accepted spend immediately');
...
q('veil-combustion').dispatchEvent(new game.window.MouseEvent('pointerdown',{bubbles:true,cancelable:true}));game.tick();game.tick();assert.equal(game.run('resources.state.tanks.fuel.amount'),methaneFuel-1);assert.equal(game.run('resources.state.tanks.oxidizer.amount'),oxygenFuel-2);assert.equal(game.run('JSON.stringify(resources.state.elements)'),elementsBeforeDrive);assert.ok(game.run('veilUI.run.driveBuffer')>1.9);assert.match(q('veil-combustion-fuel').textContent,/CH₄ \d+ · O₂ \d+/);assert.match(q('veil-combustion-remaining').textContent,/\d+ \/ 18 PACKETS · \d+s/);assert.match(q('veil-combustion-meter').style.transform,/scaleX\([\d.]+\)/);
assert.equal(q('veil-thermal').hidden,false);assert.equal(q('veil-thermal-state').textContent,'NORMAL');for(let i=0;i<220;i++)game.tick();assert.equal(q('veil-thermal').dataset.state,'cooling');assert.match(q('veil-coolant').textContent,/AUTO H₂O · 7/);assert.equal(game.run('resources.state.tanks.coolant.amount'),7);assert.equal(game.run('veilUI.run.fuel.coolant.amount'),7);const bufferAtLock=game.run('veilUI.run.driveBuffer');q('veil-return').click();assert.equal(game.run('veilUI.returning'),'locking');assert.equal(game.run('veilUI.run.driveHeld'),false,'Starting ANCHOR LOCK releases held COMBUSTION');assert.equal(game.run('veilUI.run.driveBuffer'),bufferAtLock,'The already paid packet remainder is preserved');
