...
q('launch-veil').click();const fuel=game.run('resources.state.molecules.hydrogen');assert.equal(q('veil-fuel').textContent,`${Math.min(3,fuel)} / 3`);assert.match(q('veil-burst-meter').style.transform,/scaleX\([\d.]+\)/);
...
assert.equal(q('veil-fuel').textContent,`${Math.max(0,Math.min(3,fuel)-1)} / 3`,'BURST gauge follows the accepted spend immediately');
...
q('veil-combustion').dispatchEvent(new game.window.MouseEvent('pointerdown',{bubbles:true,cancelable:true}));game.tick();game.tick();assert.equal(game.run('resources.state.molecules.methane'),methaneFuel-1);assert.equal(game.run('resources.state.molecules.oxygen'),oxygenFuel-2);assert.ok(game.run('veilUI.run.driveBuffer')>1.9);assert.match(q('veil-combustion-fuel').textContent,/CH₄ \d+ · O₂ \d+/);assert.match(q('veil-combustion-remaining').textContent,/\d+ \/ 18 PACKETS · \d+s/);assert.match(q('veil-combustion-meter').style.transform,/scaleX\([\d.]+\)/);q('veil-combustion').dispatchEvent(new game.window.MouseEvent('pointerup',{bubbles:true}));
