...
q('open-supply').click();assert.equal(q('supply-dialog').open,true);assert.equal(q('open-supply').textContent.trim(),'収集殻');assert.equal(q('shell-propellant').querySelector('small').textContent,'H₂');assert.equal(q('tank-model-name').textContent,'H₂ · 水素');assert.equal(q('tank-comparison').children.length,2);
...
q('element-palette').querySelector('[data-element="H"]').click();game.run('updateStructureFrame(performance.now()+1000)');game.run('clearField()');assert.equal(q('craft-target').hidden,false,'Manual cleanup retains the craft target');
q('open-supply').click();q('launch-veil').click();assert.equal(q('craft-target').hidden,true,'Launching exploration clears the craft target');const fuel=game.run('resources.state.tanks.propellant.amount');assert.equal(game.run('veilUI.run.map.depletion.H'),inventoryDepletion(JSON.parse(game.run('JSON.stringify(resources.state.elements)')),'H'),'Launch uses the actual base inventory');assert.equal(q('veil-boost').getAttribute('aria-description'),'残り噴射 3回');assert.match(q('veil-burst-meter').style.transform,/scaleX\([\d.]+\)/);
...
console.log('Production H Veil UI passed: Collector Shell launch, manual H₂ design, fixed hold fill, explicit encyclopedia/craft routes, target cleanup, boost spending, multitouch cancellation, blur pause, exact resource/workspace reload and refunds. Canvas/WebGL are stubbed; this is not a visual or subjective playtest.');
...
assert.equal(game.run('resources.state.elements.H'),0);assert.equal(game.run("Object.hasOwn(resources.state,'molecules')"),false);assert.equal(game.run('resources.state.recipes.length'),0);assert.equal(game.run('molecule.atoms.length'),0);assert.equal(game.run('collectionGame.state.discoveredCount'),0);assert.equal(q('craft-tank-actions').hidden,true);
...
assert.equal(q('veil-thermal').hidden,false);assert.equal(q('veil-thermal').dataset.state,'normal');for(let i=0;i<220;i++)game.tick();assert.equal(q('veil-thermal').dataset.state,'cooling');assert.match(q('veil-coolant').textContent,/❄ H₂O/);assert.equal(game.run('resources.state.tanks.coolant.amount'),7);assert.equal(game.run('veilUI.run.fuel.coolant.amount'),7);const bufferAtLock=game.run('veilUI.run.driveBuffer');q('veil-return').click();assert.equal(game.run('veilUI.returning'),'locking');assert.equal(game.run('veilUI.run.driveHeld'),false,'Starting ANCHOR LOCK releases held COMBUSTION');assert.equal(game.run('veilUI.run.driveBuffer'),bufferAtLock,'The already paid packet remainder is preserved');
...
console.log('C/O palette gating, paid handmade CH₄ template, use-specific molecule models and comparison bars, direct capped tanks, automatic H₂O cooling, independent H₂ BURST, no environmental coolant gate and captured return passed.');
...
assert.match(q('collection-detail').textContent,/満載9回/);
[...q('collection-detail').querySelectorAll('button')].find(b=>b.textContent==='補給で比較する').click();assert.equal(q('collection-dialog').open,false);assert.equal(q('supply-dialog').open,true);assert.match(q('tank-model-name').textContent,/CO[₂2]/);
...
// CHO ending is reached spatially and banked only by a successful safe return.
