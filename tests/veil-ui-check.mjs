...
const reachSafeSite=()=>{game.window.dispatchEvent(new game.window.KeyboardEvent('keyup',{key:'ArrowUp'}));game.window.dispatchEvent(new game.window.KeyboardEvent('keyup',{key:'ArrowDown'}));game.window.dispatchEvent(new game.window.KeyboardEvent('keyup',{key:'ArrowLeft'}));game.window.dispatchEvent(new game.window.KeyboardEvent('keyup',{key:'ArrowRight'}));game.window.dispatchEvent(new game.window.KeyboardEvent('keyup',{key:'Shift'}));game.run(`(()=>{const run=veilUI.run,site=run.map.safeExtractionSite;Object.assign(run.player,{x:site.x,y:site.y,vx:0,vy:0,boost:0,combustion:false});run.driveHeld=false;})()`);for(let i=0;i<7;i++)game.tick();};
...
assert.equal(game.run('veilUI.returnPhase'),'exploring');assert.equal(q('veil-return').disabled,true,'Insight-less sortie cannot extract from the spawn');q('veil-return').click();assert.equal(game.run('veilUI.returnPhase'),'exploring','The disabled return affordance does not begin extraction');
reachSafeSite();assert.equal(q('veil-return').dataset.returnState,'site-ready');assert.equal(q('veil-return').disabled,false);assert.equal(q('veil-view').hidden,false,'Arriving at the Safe Extraction Site does not return automatically');
...
const safeReturnTime=game.run('veilUI.run.time'),sitePosition=game.run('({x:veilUI.run.player.x,y:veilUI.run.player.y})');q('veil-return').click();assert.equal(game.run('veilUI.returnPhase'),'normal-extraction-pending');assert.equal(game.run('veilUI.returning'),'normal-extraction-pending');assert.equal(game.run('veilUI.run.returnEffect.mode'),'stable');assert.equal(q('veil-return-label').textContent,'EXTRACTING');assert.equal(game.run('resources.state.elements.H'),0,'Normal extraction does not settle cargo before its wait phase');assert.equal(q('veil-return').disabled,true,'Normal extraction disables repeat input');for(let i=10;i--;)game.tick();q('veil-return').click();assert.ok(game.run('veilUI.run.time')>safeReturnTime,'Normal extraction advances the field clock');assert.deepEqual(game.run('({x:veilUI.run.player.x,y:veilUI.run.player.y})'),sitePosition,'The ship stays in place during normal extraction');assert.equal(game.run('veilUI.run.testPendingPickup.ready'),0,'Pending extraction does not collect nearby dust');assert.ok(game.run('veilUI.run.eaters[0].x')<game.run('veilUI.run.testEaterStartX'),'Dust Eater remains in motion during normal extraction');assert.equal(game.run('veilUI.run.captured'),false,'A Dust Eater cannot start a competing forced return once extraction has begun');assert.notEqual(q('veil-extraction-meter').style.transform,'scaleX(0)');finishReturn();assert.equal(game.run('resources.settlementCalls'),1,'The normal settlement authority runs once');game.run('resources.settleExpedition=resources.__originalSettlement;delete resources.__originalSettlement;delete resources.settlementCalls;');assert.equal(q('veil-view').hidden,true);assert.equal(game.run('resources.state.elements.H'),gathered);
// The ship-mounted Insight bulb is a real touch button and starts extraction anywhere.
q('open-supply').click();game.run("veilUI.requestExpeditionLaunch('continue')");await settle();game.run("veilUI.run.carriedInsights=['ethane']");for(let i=0;i<6;i++)game.tick();const insightBulb=game.document.querySelector('.veil-normal-insight');assert.ok(insightBulb instanceof game.window.HTMLButtonElement);assert.equal(q('veil-return').dataset.returnState,'insight-ready');assert.equal(q('veil-return').disabled,false);insightBulb.click();assert.equal(game.run('veilUI.returnPhase'),'normal-extraction-pending','Tapping the ship bulb begins normal extraction away from the site');assert.equal(q('veil-return').disabled,true);assert.equal(insightBulb.disabled,true,'Insight return cannot be triggered again while extraction is pending');
...
q('open-supply').click();game.run("veilUI.requestExpeditionLaunch('continue')");await settle();assert.equal(game.run("veilUI.run.carriedInsights.includes('ethane')"),false,'Previous-run Insight does not enter the next sortie');reachSafeSite();q('veil-return').click();finishReturn();assert.equal(q('veil-view').hidden,true);
...
q('open-supply').click();game.run("veilUI.requestExpeditionLaunch('continue')");game.window.dispatchEvent(new game.window.KeyboardEvent('keydown',{key:'ArrowUp'}));for(let i=0;i<200;i++)game.tick();assert.ok(game.run('veilUI.run.collectedElements.H')>0);assert.equal(game.run('resources.state.elements.H'),0);reachSafeSite();q('veil-return').click();finishReturn();assert.ok(game.run('resources.state.elements.H')>0);
...
game.run("veilUI.requestExpeditionLaunch('continue')");await new Promise(resolve=>setTimeout(resolve,540));assert.equal(game.run('resources.state.tanks.propellant.molecule'),'carbon-dioxide');assert.equal(game.run('resources.state.tanks.propellant.amount'),72);assert.equal(q('veil-boost').getAttribute('aria-description'),'残り噴射 9回');q('veil-boost').click();q('veil-return').click();finishReturn();assert.match(q('craft-last-run').textContent,/↩/);
