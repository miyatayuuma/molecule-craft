...
game.run("for(let i=0;i<120;i++)veilUI.directFill('propellant','hydrogen')");
assert.equal(game.run('resources.state.elements.H'),hBefore-240);assert.equal(game.run('resources.state.molecules.hydrogen'),0);assert.deepEqual(game.run('resources.state.tanks.propellant'),{molecule:'hydrogen',amount:120});assert.equal(game.run('molecule.atoms.length'),2,'The handmade molecule remains as the production template');
...
q('open-supply').click();q('launch-veil').click();const fuel=game.run('resources.state.tanks.propellant.amount');assert.equal(q('veil-fuel').textContent,'3 / 3');assert.match(q('veil-burst-meter').style.transform,/scaleX\([\d.]+\)/);
q('veil-boost').dispatchEvent(new game.window.MouseEvent('pointerdown',{bubbles:true,cancelable:true}));
q('veil-boost').dispatchEvent(new game.window.MouseEvent('pointerdown',{bubbles:true,cancelable:true}));
assert.equal(game.run('resources.state.tanks.propellant.amount'),fuel-40,'Cooldown blocks duplicate tank spend');assert.equal(game.run('resources.state.molecules.hydrogen'),0,'BURST does not touch BASE STOCK');
...
game.run(`resources.collect({H:400,C:30,O:80},0);resources.discover('hydrogen');resources.discover('oxygen');resources.discover('water');for(let i=0;i<120;i++)resources.fillTankFromElements('propellant','hydrogen');for(let i=0;i<18;i++)resources.fillTankFromElements('fuel','methane');for(let i=0;i<36;i++)resources.fillTankFromElements('oxidizer','oxygen');resources.visit('oxygen');veilUI.updateCraft();`);
