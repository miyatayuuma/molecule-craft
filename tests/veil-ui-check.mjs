...
q('open-supply').click();assert.equal(q('supply-dialog').open,true);assert.equal(q('supply-h2-current').textContent,'0');assert.equal(q('fill-hydrogen').disabled,true);assert.match(q('supply-h2-detail').textContent,/補給可能 0 \/ 必要 3 · 不足 H₂ ×3/);
const hBefore=game.run('resources.state.elements.H');q('craft-hydrogen-shortage').click();assert.equal(q('production-quantity').value,'3');q('make-h2').click();assert.equal(game.run('resources.state.elements.H'),hBefore-6);assert.equal(game.run('resources.state.molecules.hydrogen'),3);await new Promise(resolve=>setTimeout(resolve,680));
...
q('veil-boost').dispatchEvent(new game.window.MouseEvent('pointerdown',{bubbles:true,cancelable:true}));const boostAtLock=game.run('veilUI.run.player.boost'),positionAtLock=game.run('veilUI.run.player.y'),tankAtLock=game.run('resources.state.tanks.hydrogen');q('veil-return').click();assert.equal(game.run('veilUI.returning'),'locking');q('veil-boost').dispatchEvent(new game.window.MouseEvent('pointerdown',{bubbles:true,cancelable:true}));assert.equal(game.run('resources.state.tanks.hydrogen'),tankAtLock,'No new BURST can start during ANCHOR LOCK');for(let i=0;i<10;i++)game.tick();assert.ok(game.run('veilUI.run.player.boost')<boostAtLock&&game.run('veilUI.run.player.boost')>0,'A BURST already in flight keeps its short inertia during the lock');assert.notEqual(game.run('veilUI.run.player.y'),positionAtLock);finishReturn();
...
console.log('Production H Veil UI passed: new save, collection, return, checked-out atom placement, manual H₂ electron drag, put-away refunds, quick synthesis, boost spending, multitouch cancellation, blur pause, exact resource/workspace reload and refunds. Canvas/WebGL are stubbed; this is not a visual or subjective playtest.');
...
assert.equal(game.run("resources.state.recipes.includes('methane')"),true);assert.equal(game.run('resources.state.molecules.methane'),0);assert.equal(game.run('molecule.atoms.length'),5);game.run('clearField()');assert.equal(game.run('molecule.atoms.length'),0);
...
game.run(`resources.collect({H:250,C:30,O:80},0);resources.discover('hydrogen');resources.discover('oxygen');resources.discover('water');resources.makeMolecule('hydrogen',3);resources.makeMolecule('methane',12);resources.makeMolecule('oxygen',36);resources.makeMolecule('water',2);resources.visit('oxygen');veilUI.updateCraft();`);
...
console.log('C/O palette gating, paid handmade CH₄ checkout/refund, quantity production, explicit capped tanks, hold/release COMBUSTION DRIVE, independent H₂ BURST, no H₂O gate and captured return passed.');
