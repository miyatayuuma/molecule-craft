...
const root=new URL('../',import.meta.url),appURL=new URL('src/app.js?v=57',root),html=await readFile(new URL('index.html',root),'utf8');
...
const finishReturn=()=>{for(let i=0;i<140;i++)game.tick();};
const tapShip=target=>{const canvas=target.document.getElementById('veil-canvas'),init={bubbles:true,cancelable:true,clientX:195,clientY:422,button:0};canvas.dispatchEvent(new target.window.MouseEvent('pointerdown',init));canvas.dispatchEvent(new target.window.MouseEvent('pointerup',init));};
const reachSafeSite=()=>{game.window.dispatchEvent(new game.window.KeyboardEvent('keyup',{key:'ArrowUp'}));game.window.dispatchEvent(new game.window.KeyboardEvent('keyup',{key:'ArrowDown'}));game.window.dispatchEvent(new game.window.KeyboardEvent('keyup',{key:'ArrowLeft'}));game.window.dispatchEvent(new game.window.KeyboardEvent('keyup',{key:'ArrowRight'}));game.window.dispatchEvent(new game.window.KeyboardEvent('keyup',{key:'Shift'}));game.run(`(()=>{const run=veilUI.run,site=run.map.safeExtractionSites[0];Object.assign(run.player,{x:site.x,y:site.y,vx:0,vy:0,boost:0,combustion:false});run.driveHeld=false;})()`);for(let i=0;i<7;i++)game.tick();};
...
q('open-supply').click();assert.equal(q('supply-dialog').open,true);game.run("veilUI.requestExpeditionLaunch('continue')");await settle();assert.equal(q('supply-dialog').open,false);assert.equal(q('veil-view').hidden,false);
...
assert.equal(game.run('veilUI.returnPhase'),'exploring');assert.equal(q('veil-return'),null,'The former unconditional return control is removed');assert.equal(q('veil-status-panel').dataset.state,'site-seeking');tapShip(game);assert.equal(game.run('veilUI.returnPhase'),'exploring','A ship tap outside a Safe Site cannot extract without Insight');
reachSafeSite();assert.equal(q('veil-status-panel').dataset.state,'site-ready');assert.equal(q('veil-view').hidden,false,'Arriving at the Safe Extraction Site does not return automatically');
game.run(`(()=>{const run=veilUI.run,p=run.player;run.map.dust.push({id:run.map.dust.length,x:p.x,y:p.y,angle:0,element:'H',kind:'normal',value:1,ready:0});run.testPendingPickup=run.map.dust.at(-1);run.eaters=[{id:77,x:p.x+49,y:p.y,angle:Math.PI,speed:178,targetSpeed:178,vx:0,vy:0,phase:0,lead:1,flank:0,trail:[]}];run.testEaterStartX=run.eaters[0].x;resources.__originalSettlement=resources.settleExpedition;resources.settlementCalls=0;resources.settleExpedition=(...args)=>{resources.settlementCalls++;return resources.__originalSettlement(...args);};})()`);
const safeReturnTime=game.run('veilUI.run.time'),sitePosition=game.run('({x:veilUI.run.player.x,y:veilUI.run.player.y})');tapShip(game);assert.equal(game.run('veilUI.returnPhase'),'normal-extraction-pending');assert.equal(game.run('veilUI.returning'),'normal-extraction-pending');assert.equal(game.run('veilUI.run.returnEffect.mode'),'stable');assert.equal(q('veil-status-panel').dataset.state,'extracting');assert.equal(game.run('resources.state.elements.H'),0,'Normal extraction does not settle cargo before its wait phase');assert.equal(q('veil-status-panel').dataset.state,'extracting','Status panel reports the pending return without accepting input');for(let i=10;i--;)game.tick();tapShip(game);assert.ok(game.run('veilUI.run.time')>safeReturnTime,'Normal extraction advances the field clock');assert.deepEqual(game.run('({x:veilUI.run.player.x,y:veilUI.run.player.y})'),sitePosition,'The ship stays in place during normal extraction');assert.equal(game.run('veilUI.run.testPendingPickup.ready'),0,'Pending extraction does not collect nearby dust');assert.ok(game.run('veilUI.run.eaters[0].x')<game.run('veilUI.run.testEaterStartX'),'Dust Eater remains in motion during normal extraction');assert.equal(game.run('veilUI.run.captured'),false,'A Dust Eater cannot start a competing forced return once extraction has begun');assert.equal(q('veil-status-panel').dataset.state,'extracting');finishReturn();assert.equal(game.run('resources.settlementCalls'),1,'The normal settlement authority runs once');game.run('resources.settleExpedition=resources.__originalSettlement;delete resources.__originalSettlement;delete resources.settlementCalls;');assert.equal(q('veil-view').hidden,true);assert.equal(game.run('resources.state.elements.H'),gathered);
// The ship is the sole extraction target; its Insight marker remains visual.
q('open-supply').click();game.run("veilUI.requestExpeditionLaunch('continue')");await settle();game.run("veilUI.run.carriedInsights=['ethane']");for(let i=6;i--;)game.tick();const insightBulb=game.document.querySelector('.veil-normal-insight');assert.ok(insightBulb instanceof game.window.HTMLSpanElement);assert.equal(q('veil-status-panel').dataset.state,'insight-ready');tapShip(game);assert.equal(game.run('veilUI.returnPhase'),'normal-extraction-pending','Tapping the ship begins normal extraction away from the site');
game.run(`(()=>{resources.__originalSettlement=resources.settleExpedition;resources.settlementCalls=0;resources.settleExpedition=(...args)=>{resources.settlementCalls++;return resources.__originalSettlement(...args);};})()`);const pendingSave=game.window.localStorage.getItem('molecule-craft.resources.v1');game.window.dispatchEvent(new game.window.Event('pagehide'));assert.equal(game.run('resources.settlementCalls'),0,'Reloading during extraction does not settle the run');game=await setup(null,0,game.window.localStorage.getItem('molecule-craft.resources.v1'));assert.equal(game.run('veilUI.active'),false,'Reload exits the in-memory pending run to BASE');assert.equal(game.run('resources.state.elements.H'),gathered,'Pending reload does not settle cargo');assert.ok(pendingSave);
q('open-supply').click();game.run("veilUI.requestExpeditionLaunch('continue')");await settle();assert.equal(game.run("veilUI.run.carriedInsights.includes('ethane')"),false,'Previous-run Insight does not enter the next sortie');reachSafeSite();tapShip(game);finishReturn();assert.equal(q('veil-view').hidden,true);
...
q('veil-boost').dispatchEvent(new game.window.MouseEvent('pointerdown',{bubbles:true,cancelable:true}));reachSafeSite();game.run('veilUI.run.player.boost=2;veilUI.run.player.vx=140');const positionAtExtraction=game.run('({x:veilUI.run.player.x,y:veilUI.run.player.y})'),tankAtExtraction=game.run('resources.state.tanks.propellant.amount');tapShip(game);assert.equal(game.run('veilUI.returnPhase'),'normal-extraction-pending');assert.equal(game.run('veilUI.run.player.boost'),0,'Starting extraction stops a BURST in progress');q('veil-boost').dispatchEvent(new game.window.MouseEvent('pointerdown',{bubbles:true,cancelable:true}));assert.equal(game.run('resources.state.tanks.propellant.amount'),tankAtExtraction,'No new BURST can start during normal extraction');for(let i=10;i--;)game.tick();assert.deepEqual(game.run('({x:veilUI.run.player.x,y:veilUI.run.player.y})'),positionAtExtraction,'The ship cannot move during normal extraction');finishReturn();
...
q('open-supply').click();game.run("veilUI.requestExpeditionLaunch('continue')");game.window.dispatchEvent(new game.window.KeyboardEvent('keydown',{key:'ArrowUp'}));for(let i=0;i<200;i++)game.tick();assert.ok(game.run('veilUI.run.collectedElements.H')>0);assert.equal(game.run('resources.state.elements.H'),0);reachSafeSite();tapShip(game);finishReturn();assert.ok(game.run('resources.state.elements.H')>0);
...
assert.equal(q('veil-thermal').hidden,false);assert.equal(q('veil-thermal').dataset.state,'normal');for(let i=0;i<220;i++)game.tick();assert.equal(q('veil-thermal').dataset.state,'cooling');assert.match(q('veil-coolant').textContent,/❄ H₂O/);assert.equal(game.run('resources.state.tanks.coolant.amount'),79);assert.equal(game.run('veilUI.run.fuel.coolant.amount'),79);reachSafeSite();const bufferAtExtraction=game.run('veilUI.run.driveBuffer');game.run('veilUI.run.driveHeld=true;veilUI.run.player.combustion=true');tapShip(game);assert.equal(game.run('veilUI.returning'),'normal-extraction-pending');assert.equal(game.run('veilUI.run.driveHeld'),false,'Starting normal extraction releases held COMBUSTION');assert.equal(game.run('veilUI.run.driveBuffer'),bufferAtExtraction,'The already paid packet remainder is preserved');
game.run(`veilUI.run.elementDust={H:20,C:14,O:7};veilUI.run.map.dust.forEach(dust=>dust.ready=Infinity);var beforeNormalPending=JSON.stringify(resources.snapshot());var eaterPlayer=veilUI.run.player;veilUI.run.eaters=[{id:0,x:eaterPlayer.x,y:eaterPlayer.y,angle:0,speed:178,vx:0,vy:0,phase:0,trail:[]}];`);game.tick();assert.equal(game.run('veilUI.returning'),'normal-extraction-pending','A contacting Dust Eater cannot replace normal extraction after it starts');assert.equal(game.run('veilUI.run.captured'),false);assert.equal(game.run('JSON.stringify(resources.snapshot())'),game.run('beforeNormalPending'),'Extraction wait does not settle resources early');finishReturn();assert.equal(q('veil-view').hidden,true);
q('open-supply').click();game.run("veilUI.requestExpeditionLaunch('continue')");await settle();game.run(`(()=>{veilUI.run.elementDust={H:20,C:14,O:7};veilUI.run.map.dust.forEach(dust=>dust.ready=Infinity);globalThis.beforeCaptureResources=JSON.stringify(resources.snapshot());const p=veilUI.run.player;veilUI.run.eaters=[{id:0,x:p.x,y:p.y,angle:0,speed:178,vx:0,vy:0,phase:0,trail:[]}];})()`);game.tick();assert.equal(game.run('veilUI.returning'),'emergency','Dust Eater contact starts the separate forced-return phase');assert.equal(game.run('veilUI.run.returnEffect.mode'),'emergency');assert.equal(game.run('veilUI.run.lostCargoEffects.length'),6,'Task 5G loss particles lead into emergency retrieval');assert.equal(game.run('JSON.stringify(resources.snapshot())'),game.run('beforeCaptureResources'),'Emergency presentation does not deduct resources before the existing settlement');tapShip(game);for(let i=0;i<20;i++)game.tick();assert.equal(q('veil-view').hidden,false);assert.equal(game.run('veilUI.returning'),'emergency','Normal return cannot replace an active emergency transition');finishReturn();assert.equal(q('veil-view').hidden,true);assert.match(q('craft-last-run').textContent,/⚠/);
...
game.run("veilUI.requestExpeditionLaunch('continue')");await new Promise(resolve=>setTimeout(resolve,540));assert.equal(game.run('resources.state.tanks.propellant.molecule'),'carbon-dioxide');assert.equal(game.run('resources.state.tanks.propellant.amount'),72);assert.equal(q('veil-boost').getAttribute('aria-description'),'残り噴射 9回');q('veil-boost').click();tapShip(game);finishReturn();assert.match(q('craft-last-run').textContent,/↩/);
...
const starterGame=await setup();starterGame.document.getElementById('open-supply').click();starterGame.run("veilUI.requestExpeditionLaunch('continue')");
starterGame.run(`veilUI.run.elementDust.H=72;veilUI.run.collectedElements.H=24;veilUI.updateCraft();`);
assert.equal(starterGame.document.getElementById('veil-craft-prompt'),null,'FIELD has no separate action beside the ship');
...
game.run(`for(const use of ['propellant','fuel','oxidizer','coolant'])resources.setLoadoutTank(use,null);`);q('open-supply').click();game.run("veilUI.requestExpeditionLaunch('continue')");
...
tapShip(game);finishReturn();assert.equal(game.run('resources.state.progress.choCompleted'),true);assert.equal(q('cho-completion'),null);assert.match(q('craft-last-run').textContent,/CHO ✓/);
