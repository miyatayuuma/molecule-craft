...
game=await setup(null,30,saved);q('open-supply').click();for(const id of ['shell-propellant','shell-fuel','shell-oxidizer','shell-coolant'])assert.ok(q(id));assert.equal(q('shell-coolant').hidden,true,'Coolant stays hidden until thermal runtime exists');assert.equal(q('molecule-select'),null);q('supply-dialog').close();
...
game.run(`resources.collect({H:400,C:30,O:80},0);resources.discover('hydrogen');resources.discover('oxygen');resources.discover('water');resources.fillTankFromElements('propellant','hydrogen',120);resources.fillTankFromElements('fuel','methane',18);resources.fillTankFromElements('oxidizer','oxygen',36);resources.visit('oxygen');veilUI.updateCraft();`);
assert.deepEqual(game.run('resources.state.tanks'),{propellant:{molecule:'hydrogen',amount:120},fuel:{molecule:'methane',amount:18},oxidizer:{molecule:'oxygen',amount:36},coolant:{molecule:null,amount:0}});
q('open-supply').click();q('shell-fuel').click();assert.equal(q('tank-model-name').textContent,'CH₄ · メタン');assert.equal(q('tank-comparison').children.length,2);q('shell-oxidizer').click();assert.equal(q('tank-model-name').textContent,'O₂ · 酸素');
...
assert.equal(q('veil-thermal'),null);const bufferAtLock=game.run('veilUI.run.driveBuffer');q('veil-return').click();assert.equal(game.run('veilUI.returning'),'locking');assert.equal(game.run('veilUI.run.driveHeld'),false,'Starting ANCHOR LOCK releases held COMBUSTION');assert.equal(game.run('veilUI.run.driveBuffer'),bufferAtLock,'The already paid packet remainder is preserved');
