...
const safeReturnTime=game.run('veilUI.run.time');q('veil-return').click();assert.equal(q('veil-view').hidden,false);assert.equal(game.run('veilUI.returning'),'stable');assert.equal(game.run('veilUI.run.returnEffect.mode'),'stable');assert.equal(game.run('resources.state.elements.H'),0,'Presentation does not settle cargo early');q('veil-return').click();for(let i=0;i<20;i++)game.tick();assert.equal(game.run('veilUI.run.time'),safeReturnTime,'Return presentation is a terminal visual state, not invulnerable gameplay');assert.equal(game.run('veilUI.returning'),'stable','Repeated return input cannot start a competing transition');finishReturn();assert.equal(q('veil-view').hidden,true);assert.equal(game.run('resources.state.elements.H'),gathered);
...
q('open-supply').click();assert.equal(q('supply-dialog').open,true);assert.equal(q('supply-h2-current').textContent,'0');assert.equal(q('fill-hydrogen').disabled,true);assert.match(q('supply-h2-detail').textContent,/補給可能 1 \/ 必要 3 · 不足 H₂ ×2/);
const hBefore=game.run('resources.state.elements.H');q('craft-hydrogen-shortage').click();assert.equal(q('production-quantity').value,'2');q('make-h2').click();assert.equal(game.run('resources.state.elements.H'),hBefore-4);assert.equal(game.run('resources.state.molecules.hydrogen'),3);await new Promise(resolve=>setTimeout(resolve,680));
assert.equal(q('fill-hydrogen').disabled,false);q('fill-hydrogen').click();assert.equal(game.run('resources.state.molecules.hydrogen'),0);assert.equal(game.run('resources.state.tanks.hydrogen'),3);assert.equal(q('supply-h2-current').textContent,'3');assert.equal(q('supply-h2-meter').style.transform,'scaleX(1)');q('supply-dialog').close();
q('launch-veil').click();const fuel=game.run('resources.state.tanks.hydrogen');assert.equal(q('veil-fuel').textContent,`${fuel} / 3`);assert.match(q('veil-burst-meter').style.transform,/scaleX\([\d.]+\)/);
q('veil-boost').dispatchEvent(new game.window.MouseEvent('pointerdown',{bubbles:true,cancelable:true}));
q('veil-boost').dispatchEvent(new game.window.MouseEvent('pointerdown',{bubbles:true,cancelable:true}));
assert.equal(game.run('resources.state.tanks.hydrogen'),fuel-1,'Cooldown blocks duplicate tank spend');assert.equal(game.run('resources.state.molecules.hydrogen'),0,'BURST does not touch BASE STOCK');
...
q('veil-return').click();finishReturn();
...
q('molecule-select').value='methane';q('molecule-select').dispatchEvent(new game.window.Event('change',{bubbles:true}));const hBeforeMethane=game.run('resources.state.elements.H'),cBeforeMethane=game.run('resources.state.elements.C');q('production-add-5').click();assert.equal(q('production-quantity').value,'6');assert.match(q('molecule-cost').textContent,/C × 6.*H × 24/);q('make-h2').click();q('make-h2').click();assert.equal(game.run('resources.state.elements.H'),hBeforeMethane-24);assert.equal(game.run('resources.state.elements.C'),cBeforeMethane-6);assert.equal(q('synthesis-formula').textContent,'CH₄');assert.ok(q('synthesis-atoms').children.length<=12);await new Promise(resolve=>setTimeout(resolve,680));
game.run(`resources.collect({H:250,C:30,O:80},0);resources.discover('hydrogen');resources.discover('oxygen');resources.discover('water');resources.makeMolecule('hydrogen',3);resources.makeMolecule('methane',11);resources.makeMolecule('oxygen',36);resources.makeMolecule('water',2);resources.visit('oxygen');veilUI.updateCraft();`);
assert.equal(q('supply-combustion-packets').textContent,'0');assert.equal(q('fill-hydrogen').disabled,false);assert.equal(q('fill-combustion').disabled,false);const baseBeforeSupply=game.run('({...resources.state.molecules})');q('fill-hydrogen').click();q('fill-combustion').click();assert.deepEqual(game.run('resources.state.tanks'),{hydrogen:3,methane:18,oxygen:36});assert.equal(game.run('resources.state.molecules.hydrogen'),baseBeforeSupply.hydrogen-3);assert.equal(game.run('resources.state.molecules.methane'),baseBeforeSupply.methane-18);assert.equal(game.run('resources.state.molecules.oxygen'),baseBeforeSupply.oxygen-36);assert.equal(q('supply-combustion-packets').textContent,'18');
...
q('launch-veil').click();assert.equal(q('veil-combustion').hidden,false);const methaneBase=game.run('resources.state.molecules.methane'),oxygenBase=game.run('resources.state.molecules.oxygen'),methaneFuel=game.run('resources.state.tanks.methane'),oxygenFuel=game.run('resources.state.tanks.oxygen'),waterFuel=game.run('resources.state.molecules.water');
q('veil-combustion').dispatchEvent(new game.window.MouseEvent('pointerdown',{bubbles:true,cancelable:true}));game.tick();game.tick();assert.equal(game.run('resources.state.tanks.methane'),methaneFuel-1);assert.equal(game.run('resources.state.tanks.oxygen'),oxygenFuel-2);assert.equal(game.run('resources.state.molecules.methane'),methaneBase);assert.equal(game.run('resources.state.molecules.oxygen'),oxygenBase);assert.ok(game.run('veilUI.run.driveBuffer')>1.9);assert.match(q('veil-combustion-fuel').textContent,/CH₄ \d+ · O₂ \d+/);assert.match(q('veil-combustion-remaining').textContent,/\d+ \/ 18 PACKETS · \d+s/);assert.match(q('veil-combustion-meter').style.transform,/scaleX\([\d.]+\)/);q('veil-combustion').dispatchEvent(new game.window.MouseEvent('pointerup',{bubbles:true}));
const h2Fuel=game.run('resources.state.tanks.hydrogen'),h2Base=game.run('resources.state.molecules.hydrogen');q('veil-boost').dispatchEvent(new game.window.MouseEvent('pointerdown',{bubbles:true,cancelable:true}));assert.equal(game.run('resources.state.tanks.hydrogen'),h2Fuel-1);assert.equal(game.run('resources.state.molecules.hydrogen'),h2Base);assert.equal(game.run('resources.state.molecules.water'),waterFuel,'H₂O is not silently consumed by exploration');assert.equal(q('veil-thermal'),null);
game.run(`veilUI.run.elementDust={H:20,C:14,O:7};veilUI.run.map.dust.forEach(dust=>dust.ready=Infinity);var beforeCaptureResources=JSON.stringify(resources.snapshot());var eaterPlayer=veilUI.run.player;veilUI.run.eaters=[{id:0,x:eaterPlayer.x,y:eaterPlayer.y,angle:0,speed:178,vx:0,vy:0,phase:0,trail:[]}];`);game.tick();assert.equal(game.run('veilUI.returning'),'emergency');assert.equal(game.run('veilUI.run.returnEffect.mode'),'emergency');assert.equal(game.run('veilUI.run.lostCargoEffects.length'),6,'Task 1 loss particles lead into emergency retrieval');assert.equal(game.run('JSON.stringify(resources.snapshot())'),game.run('beforeCaptureResources'),'Emergency presentation does not deduct resources before the existing settlement');const capturedTime=game.run('veilUI.run.time');q('veil-return').click();for(let i=0;i<20;i++)game.tick();assert.equal(q('veil-view').hidden,false);assert.equal(game.run('veilUI.run.time'),capturedTime);assert.equal(game.run('veilUI.returning'),'emergency','Normal return cannot replace an active emergency transition');finishReturn();assert.equal(q('veil-view').hidden,true);assert.match(q('craft-last-run').textContent,/捕獲帰還/);
console.log('C/O palette gating, paid handmade CH₄ storage, quantity production, explicit capped tanks, hold/release COMBUSTION DRIVE, independent H₂ BURST, no H₂O gate and captured return passed.');
