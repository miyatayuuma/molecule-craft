...
try{
  ...
  const runScenario=async metrics=>{
    ...
    return evaluate(`(async()=>{const {nextCraftBondHint}=await import(${JSON.stringify(moduleUrl)});const records=${fixtureJson};const byId=id=>records.find(record=>record.id===id);const workspace=(atoms,bonds=[])=>({atoms:atoms.map((element,index)=>({id:100+index,element})),bonds:bonds.map(([a,b,order])=>({a:100+a,b:100+b,order}))});const timed=(target,current)=>{const start=performance.now(),hint=nextCraftBondHint(target,current);return {elapsed:performance.now()-start,hint};};const target=byId('methylcyclohexane'),timings=[];for(let hydrogen=0;hydrogen<=14;hydrogen++){const result=timed(target,workspace([...Array(7).fill('C'),...Array(hydrogen).fill('H')]));if(!result.hint)throw new Error('missing methylcyclohexane hint at H='+hydrogen);timings.push(result.elapsed);}const full=workspace(target.atoms),first=timed(target,full);const bonded=workspace(target.atoms,[[first.hint.workspaceIndices[0],first.hint.workspaceIndices[1],1]]),afterBond=timed(target,bonded),afterUndo=timed(target,full);const switchedTarget=byId('benzene'),switched=timed(switchedTarget,workspace(switchedTarget.atoms));const peers={};for(const id of ['methylcyclohexane','cyclohexane','benzene','n-hexane']){const record=byId(id),result=timed(record,workspace(record.atoms));if(!result.hint)throw new Error('missing '+id+' hint');peers[id]=result.elapsed;}await new Promise(resolve=>requestAnimationFrame(()=>resolve()));return {maxIncremental:Math.max(...timings),full:first.elapsed,afterBond:afterBond.elapsed,afterUndo:afterUndo.elapsed,switched:switched.elapsed,peers};})()`);
  };
  ...
  for(const [name,result] of Object.entries({desktop,mobile})){
    const samples=[result.maxIncremental,result.full,result.afterBond,result.afterUndo,result.switched,...Object.values(result.peers)];
    assert.ok(Math.max(...samples)<250,`${name} CRAFT hint refresh exceeded 250ms: ${JSON.stringify(result)}`);
  }
  console.log(`CRAFT hint Chromium regression passed: desktop max ${Math.max(desktop.maxIncremental,desktop.full).toFixed(2)}ms; mobile-equivalent max ${Math.max(mobile.maxIncremental,mobile.full).toFixed(2)}ms.`);
}finally{
