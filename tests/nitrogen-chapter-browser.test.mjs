...
try{
  ...
  await waitFor(`!!document.querySelector('#collector-launch-handle')&&!document.querySelector('#open-supply')?.disabled`,'LOADOUT UI did not become ready');await evaluate(`(()=>{document.querySelector('#open-supply').click();document.querySelector('#collector-launch-handle').dispatchEvent(new KeyboardEvent('keydown',{key:'Enter',bubbles:true}));return true;})()`);await waitFor(`document.querySelector('#expedition-destinations')?.getAttribute('aria-hidden')==='false'&&!!document.querySelector('#expedition-destinations [data-region="nitrogen"]')`,'Nitrogen destination unavailable');await evaluate(`document.querySelector('#expedition-destinations [data-region="nitrogen"]').click()`);await waitFor(`document.querySelector('#veil-view')?.hidden===false&&!!globalThis.__nitrogenChapterRun`,'Nitrogen relaunch failed');const respawn=await evaluate(`globalThis.__nitrogenChapterRun.rareSurvey.anomalies.some(a=>a.id==='rare-cl-nitrogen-pocket')`);assert.equal(respawn,false,'claimed Cl must not respawn after relaunch');
  assert.equal(exceptions.length,0,`Nitrogen chapter browser acceptance must not throw: ${JSON.stringify(exceptions)}`);
  console.log('Nitrogen chapter Chromium acceptance',JSON.stringify({zones:authored.zones.map(z=>({kind:z.kind,width:z.width})),hazards:authored.hazards.length,sectionHashes:hashes,travelled:{time:travelled.time,N:travelled.N,distance:+travelled.distance.toFixed(1)},cl:{distanceFromPocket:+basinDistance.toFixed(1),claimed:true}}));
...
console.log('Nitrogen chapter Chromium regression passed: five broad rendered phases, hazard-composition presentation, real WASD Entry-to-Mid N collection, N2 side pocket, Rare Cl claim and non-respawn relaunch.');
