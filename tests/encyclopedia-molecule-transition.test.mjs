...
test('Graph -> Detail paints the proxy in the transition portal before source handoff',async()=>{
  ...
  const handle=controller.begin({id:'a',direction:'to-detail',sourceVisual:source});assert.ok(handle);assert.equal(handle.proxy.parentNode,portal,'proxy must live in the same top-layer portal as the modal Encyclopedia');assert.deepEqual(controller.snapshot(),{phase:'preparing',moleculeId:'a',direction:'to-detail',owner:'transition',active:true});assert.equal(source.style.visibility,undefined,'source stays visible until proxy readiness is established');
  ...
  const pending=controller.attach(handle,{targetVisual:target,targetRect:{left:330,top:170,width:220,height:179}});assert.equal(controller.snapshot().phase,'opening');assert.equal(controller.snapshot().owner,'transition');await pending;
