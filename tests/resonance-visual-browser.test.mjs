...
try{
  ...
  // Encyclopedia: use the public open path, then the real Detail -> Graph gesture.
  ...
  for(const id of ['ozone','nitrobenzene','2-4-6-trinitrotoluene']){
    assert.equal(await evaluate(`window.__resonanceCollection.openMolecule(${JSON.stringify(id)})`),true,`${id}: public Encyclopedia open failed`);
    await waitFor(`document.querySelector('#collection-dialog')?.open`,'Encyclopedia dialog did not open');
    await waitFor(`document.querySelector('.molecule-detail-return[data-molecule-id=${JSON.stringify(id)}]')?.dataset.viewerReady==='true'`,`${id}: Detail viewer did not become ready`,{attempts:180,delay:50});
    ...
    await returnToGraph(id);await assertGraphVisual(id);await capture(`mobile-graph-${id}`);
  }
