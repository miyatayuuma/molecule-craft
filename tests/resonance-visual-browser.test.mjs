...
try{
  ...
  const focusBySearch=async(name,id)=>{await evaluate(`(()=>{const input=document.querySelector('.graph-search');input.value=${JSON.stringify(name)};input.dispatchEvent(new Event('input',{bubbles:true}));input.dispatchEvent(new Event('change',{bubbles:true}));return true;})()`);await waitFor(`document.querySelector('.graph-node.focus')?.dataset.graphId===${JSON.stringify(id)}`,`Graph did not focus ${id}`);await new Promise(r=>setTimeout(r,650));};
  ...
  for(const [id,name] of [['ozone','Ozone'],['nitrobenzene','Nitrobenzene'],['2-4-6-trinitrotoluene','2,4,6-Trinitrotoluene']]){
    await focusBySearch(name,id);await assertGraphVisual(id);await capture(`mobile-graph-${id}`);
    ...
    await evaluate(`window.__resonanceCollection.openMolecule('ozone')`);await new Promise(r=>setTimeout(r,80));await evaluate(`document.querySelector('#detail-back')?.click()`);await new Promise(r=>setTimeout(r,80));
  }
