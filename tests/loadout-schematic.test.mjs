...
assert.match(workstation,/assets\/loadout-v2/);assert.match(workstation,/loadout-craft-image/);assert.match(workstation,/loadout-fixed-dock-image/);assert.match(workstation,/syncLoadoutHardwareLayout/);assert.match(workstation,/setLoadoutFlightCraftTranslation/);assert.match(workstation,/collector-shell-preview/);assert.doesNotMatch(workstation,/width:145%|width:175%/);assert.match(workstation,/loadout-craft-drag-left/,'launch handle must follow the visible flight craft anchor');assert.match(workstation,/for\(const kind of \['drive'\]\)/,'PULSE and SHOCK must not receive duplicate module labels');assert.match(workstation,/background:#a7e2eb!important/,'DRIVE meters must use the shared cyan accent');assert.match(workstation,/opacity:1!important/,'LOADOUT flight craft preview must remain visible');
