...
test('Theme F post-Awakening integrated economy remains production-green',()=>{
  ...
  assert.deepEqual(Object.values(report.progression),['PASS','PASS','PASS','PASS','PASS','PASS','PASS',report.progression.softLockChecks]);
  assert.deepEqual(report.destinationRotation.balanced48,{veil:12,carbon:12,oxygen:12,nitrogen:12});
  for(const [element,row] of Object.entries(report.rareEconomy)){assert.ok(row.lowStock.selected>=1,`${element} has no low-stock supply`);assert.ok(row.replacedOrdinary>=0&&row.replacementFraction<=row.maxReplacementFraction,`${element} exceeded ordinary-resource replacement cap`);}
  assert.ok(report.rareEconomy.P.highStock.selected<=report.rareEconomy.P.lowStock.selected);
  assert.ok(report.shockAndAnchor.nitromethane.event.remaining===2);
  assert.equal(report.shockAndAnchor.oneWaveMultiPurpose,true);
  assert.ok(report.shockAndAnchor.intact.rawElectricalIntensity>report.shockAndAnchor.fractured.rawElectricalIntensity);
  assert.ok(report.shockAndAnchor.fractured.rawElectricalIntensity>0);
