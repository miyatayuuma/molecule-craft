...
assert.match(graphViewSource,/const nodeCircles=\(\)=>\[\.\.\.stage\.querySelectorAll\('\.graph-node'\)\]/,'visible-gap clipping must derive blockers from every rendered graph node');
assert.match(graphViewSource,/circles\.some\(circle=>\[armA,tip,armB\]\.some\(point=>Math\.hypot\(point\.x-circle\.x,point\.y-circle\.y\)<circle\.radius\+Math\.max\(0,padding\)\)\)/,'Chevron vertices receive a final collision check against every displayed node circle');
assert.doesNotMatch(graphViewSource,/GRAPH_MOTION_CHEVRON_CLEARANCE/,'motion clipping must not add an oversized second safety radius once displayed circles are mapped through the exact SVG screen transform');
assert.match(graphViewSource,/runGraphGeometryMotion\(graphMotion,geometryTweens,edgeTweens,chevronEdges,\{win,nodeCircles\}\)/,'normal reroot must use the same exact displayed-circle authority as reduced/suppressed final rendering');
assert.match(graphViewSource,/for\(const tween of edgeTweens\)applyGraphEdgeTween\(tween,edgeEased\);[\s\S]*updateGraphChevrons\(chevronRecords,nodeCircles\?\.\(edgeEased\)\?\?null\)/,'Every RAF must update edge geometry first and derive every Chevron from those current line endpoints and the same edge progress in the same frame');
assert.match(graphViewSource,/updateGraphChevrons\(chevronEdges,nodeCircles\(\)\);runGraphGeometryMotion\(graphMotion,geometryTweens,edgeTweens,chevronEdges/,'A safe Chevron must be initialized against the displayed node circles before the first RAF, never hidden until motion starts');
assert.doesNotMatch(graphViewSource,/for\(const record of chevronEdges\)record\.chevron\?\.setAttribute\('visibility','hidden'\)/,'Chevron motion must not begin with a hidden marker that appears on a later animation frame');
