...
assert.match(graphViewSource,/const movingLines=new Set\(edgeTweens\.map\(tween=>tween\.line\)\);updateGraphChevrons\(chevronEdges\.filter\(record=>!movingLines\.has\(record\.line\)\),nodeCircles\(\)\);runGraphGeometryMotion/,'Moving chevrons must stay hidden until the first synchronized edge/node geometry frame replaces stale coordinates');
