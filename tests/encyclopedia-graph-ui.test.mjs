...
for(const edge of [chainIntoButane,chainOutOfButane]){
  ...
  const expectedAnchor={x:geometry.start.x+(geometry.end.x-geometry.start.x)*.6,y:geometry.start.y+(geometry.end.y-geometry.start.y)*.6};assert(Math.hypot(geometry.anchor.x-expectedAnchor.x,geometry.anchor.y-expectedAnchor.y)<.01,`${edge.from} → ${edge.to}: directional Chevron anchor must stay inside the current safe segment while avoiding an invariant midpoint`);
