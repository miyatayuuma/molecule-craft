...
for(const id of ['cyclopentane','tetrahydrofuran']){
  ...
  assert.ok(initial.fiveMemberConformationRelative<.12,`${id}: baseline pucker authority did not converge (${initial.fiveMemberConformationRelative})`);
  ...
  assert.ok(errors.fiveMemberConformationRelative<.12,`${id}: pucker authority did not recover after perturbation (${errors.fiveMemberConformationRelative})`);
