...
test('current-schema save without history normalizes without resetting player progress',()=>{
  assert.equal(SCHEMA_VERSION,9);const original=createInitialResourcesState();Object.assign(original.progress,{choCompleted:true,regions:['veil','carbon','oxygen','nitrogen'],checkpoint:'nitrogen',foundElements:['H','C','O','N'],insightSeed:{id:'candidate-a',hotDestination:'carbon'}});original.recipes=['hydrogen','root'];original.hints=['candidate-b'];Object.assign(original.elements,{H:370,C:314,O:830,N:168});original.tanks.propellant={molecule:'hydrogen',amount:17};delete original.progress.insightDestinationHistory;
  const storage=memory([[RESOURCE_KEY,JSON.stringify(original)]]),loaded=loadPersistedResources(storage),state=loaded.state;
  assert.deepEqual(state.progress.insightDestinationHistory,[]);assert.equal(state.progress.choCompleted,true);assert.equal(state.progress.checkpoint,'nitrogen');assert.deepEqual(state.recipes,['hydrogen','root']);assert.deepEqual(state.hints,['candidate-b']);assert.deepEqual(state.elements,{...original.elements});assert.deepEqual(state.tanks.propellant,{molecule:'hydrogen',amount:17});assert.equal(Object.hasOwn(state,'upgrades'),false);assert.deepEqual(state.progress.insightSeed,original.progress.insightSeed);
