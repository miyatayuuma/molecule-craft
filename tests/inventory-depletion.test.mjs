...
test('stock changes particle presence without moving landmarks or currents',()=>{
  const totals={};for(const seed of [1,71,991]){
    ...
    assert.deepEqual(a.signals,b.signals);assert.deepEqual(a.fields,b.fields);
    const routeGeometry=routes=>routes.map(({id,points})=>({id,points}));assert.deepEqual(routeGeometry(a.routes),routeGeometry(b.routes));
