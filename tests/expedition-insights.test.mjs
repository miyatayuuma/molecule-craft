...
// H₂O needs completed CH₄ + O₂, eight coolant molecules worth of atoms, and
// authored high-thermal COMBUSTION exposure. Generic strain/overheat is insufficient.
{
  const value=resources(),cost=starterCost(value,'water');assert.deepEqual(cost,{O:8,H:16});value.discover('methane');value.discover('oxygen');value.findElementForExpedition('O');Object.assign(value.state.elements,cost);assert.ok(!value.progressionInsightCandidates().includes('water'));value.state.progress.thermalStrainExperienced=true;assert.ok(!value.progressionInsightCandidates().includes('water'));assert.equal(value.recordDriveThermalInterruption(),true);assert.ok(!value.progressionInsightCandidates().includes('water'));assert.equal(value.recordDriveThermalInterruption(),true);assert.ok(!value.progressionInsightCandidates().includes('water'));assert.equal(value.recordCoolantNeedExperience(),true);value.state.elements.O--;assert.ok(!value.progressionInsightCandidates().includes('water'));value.state.elements.O++;assert.ok(value.progressionInsightCandidates().includes('water'));
  const oOnly=resources();oOnly.findElementForExpedition('O');Object.assign(oOnly.state.elements,cost);oOnly.state.progress.thermalStrainExperienced=true;oOnly.recordDriveThermalInterruption();oOnly.recordDriveThermalInterruption();oOnly.recordCoolantNeedExperience();assert.ok(!oOnly.progressionInsightCandidates().includes('water'));
}

// Thermal/DRIVE and authored coolant-need experience survive capture while the
// H₂O idea remains run-local and can be offered again from BASE STOCK next launch.
{
  const value=resources(),cost=starterCost(value,'water');value.discover('methane');value.discover('oxygen');Object.assign(value.state.elements,cost);assert.equal(value.recordThermalStrain(),true);assert.equal(value.recordDriveThermalInterruption(),true);assert.equal(value.recordDriveThermalInterruption(),true);assert.equal(value.recordCoolantNeedExperience(),true);assert.deepEqual(value.progressionInsightCandidates(),['water']);const flight=run();assert.deepEqual(triggerInsight(flight,'water',value.state),{type:'insightReady',id:'water',critical:true});settle(value,flight,true);assert.equal(value.state.progress.thermalStrainExperienced,true);assert.equal(value.state.progress.driveThermalInterruptions,2);assert.equal(value.state.progress.coolantNeedExperienced,true);assert.ok(!value.state.hints.includes('water'));assert.deepEqual(value.progressionInsightCandidates(),['water']);
