...
assert.equal(EXPEDITION.normalExtractionSeconds,.8);assert.ok(EXPEDITION.normalExtractionSeconds<=1,'Normal extraction stays a short pending phase');assert.ok(DRIVES.hydrogen.boostSpeed*DRIVES.hydrogen.boostSeconds>EXPEDITION.eaterSpeed*EXPEDITION.normalExtractionSeconds*3,'BURST remains a meaningful escape tool while exploring');
