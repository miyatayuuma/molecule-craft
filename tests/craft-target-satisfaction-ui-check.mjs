...
const missingElement=element=>q('craft-target-atoms').querySelector(`[data-element="${element}"]`);
const remaining=()=>game.run("JSON.stringify(targetPartsFor(resources.record(craftTargetId)).map(p=>[p.partId,p.element??null,p.atomIndices]))");
...
game.run('updateStructureFrame(2000);molecule.removeBond(molecule.atoms[0].id,molecule.atoms[1].id);topologyChanged();refreshInfo();');assert.equal(oh(),null,'breaking a bond must not make already secured O/H material reappear');assert.equal(remaining(),fromTarget);
game.run('molecule.setBond(molecule.atoms[0].id,molecule.atoms[1].id,1);topologyChanged();refreshInfo();');assert.equal(oh(),null);assert.equal(remaining(),fromTarget);
game.run('clearField({silent:true});');assert.ok(oh(),'removing workspace atoms releases their target material reservation');
...
game.run('updateStructureFrame(2000);clearField({silent:true});addElement("O");updateStructureFrame(2000);addElement("H");updateStructureFrame(2000);');assert.equal(oh(),null,'loose O + H already secure the hydroxyl material shortcut');const looseRemaining=remaining();
game.run('molecule.setBond(molecule.atoms[0].id,molecule.atoms[1].id,1);topologyChanged();refreshInfo();');assert.equal(remaining(),looseRemaining,'bonding secured atoms does not change material shortage');
...
const restored=await setup(null,0,resourceSaved,collectionSaved);assert.equal(restored.run("JSON.stringify(targetPartsFor(resources.record(craftTargetId)).map(p=>[p.partId,p.element??null,p.atomIndices]))"),looseRemaining);assert.equal(restored.document.querySelector('#craft-target-atoms [data-part-id="hydroxyl"]'),null);
restored.run('craftWorkspace.removeAtom(molecule.atoms[0].id);selectAtom(null);topologyChanged();refreshInfo();');assert.equal(restored.document.querySelector('#craft-target-atoms [data-part-id="hydroxyl"]'),null,'a partially secured part must not re-request atoms still present in workspace');assert.ok(restored.document.querySelector('#craft-target-atoms [data-element="O"]'),'only the deleted oxygen returns as an exact atomic shortage');
console.log('Target material DOM passed: workspace composition, not transient bond topology, owns shortage state across break/rebond/delete and reload.');
