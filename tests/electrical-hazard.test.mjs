...
const center={x:ELECTRICAL_FIELD.lobes[1].x,y:ELECTRICAL_FIELD.lobes[1].y},safe={x:250,y:center.y};
...
const makeRun=(position=center)=>{const state=awakenedState(),config=flightConfig(state),map=createUniverse(73,state.elements,{capabilities}),run=createRun(map,config,{predators:false});Object.assign(run.player,{x:position.x,y:position.y,angle:-Math.PI/2,vx:0,vy:-config.speed,speed:config.speed});return run;};
...
test('Electrical production field is post-Awakening only, deterministic and organic',()=>{
  const base=createUniverse(73,{H:0,C:0,O:0,N:0},{capabilities:{combustionDrive:true,nitrogenField:true}}),awakened=createUniverse(73,{H:0,C:0,O:0,N:0,P:0,S:0,F:0,Cl:0},{capabilities});
  assert.equal(environmentAt(center,0,base).hazards.some(item=>item.type==='electrical'),false);
  const centerHazard=environmentAt(center,0,awakened).hazards.find(item=>item.type==='electrical');assert.ok(centerHazard&&centerHazard.subtype==='charged-region');assert.equal(centerHazard.id,ELECTRICAL_FIELD.id);
  ...
  const otherSeed=createUniverse(991,{H:0,C:0,O:0,N:0,P:0,S:0,F:0,Cl:0},{capabilities});
  assert.equal(environmentAt(center,0,otherSeed).hazards.find(item=>item.type==='electrical').effectiveIntensity,centerHazard.effectiveIntensity);
  assert.deepEqual(ELECTRICAL_FIELD.lobes.map(lobe=>lobe.id),['entry','core','branch','tail']);
});

test('Electrical placement preserves a safe main-route choice and avoids Abrasive / BURST shear cores',()=>{
  assert.equal(electricalSpatialAt({x:-380,y:-5340}),0);
  const carbonShear=BURST_ADVANTAGE_FIELDS.find(field=>field.id==='carbon-sweep-shear');assert.ok(carbonShear);
  assert.equal(electricalSpatialAt(carbonShear),0);assert.equal(abrasiveSpatialAt(center),0);
  assert.ok(Math.hypot(center.x-ABRASIVE_PLUME.lobes[0].x,center.y-ABRASIVE_PLUME.lobes[0].y)>700);
});

test('Electrical raw intensity directly controls steering and propulsion response',()=>{
  const run=makeRun();stepRun(run,{x:1,y:0},1/60,{});const intensity=electricalIntensity(run),response=electricalResponseFor(intensity);
  assert.ok(intensity>0);assert.equal(run.treatments,undefined);assert.equal(run.hazardEffectMultipliers,undefined);
  assert.equal(run.electricalControlAuthority,response.controlAuthority);assert.equal(run.electricalPropulsionAuthority,response.propulsionAuthority);
  assert.ok(run.electricalControlAuthority<1&&run.electricalPropulsionAuthority<1);
  const config=run.config,normal=createFlight(config),stressed=createFlight(config);
  ...
  assert.ok(Math.abs(stressed.angle+Math.PI/2)<Math.abs(normal.angle+Math.PI/2));
  for(const player of [normal,stressed])Object.assign(player,{angle:-Math.PI/2,vx:0,vy:0,speed:config.driftSpeed});
  moveFlight(normal,{x:0,y:-1},.2,{config,environment:{controlAuthority:1,propulsionAuthority:1}});moveFlight(stressed,{x:0,y:-1},.2,{config,environment:response});assert.ok(stressed.speed<normal.speed);
});

test('Electrical penalty clears outside its production field',()=>{
  const run=makeRun(center);stepRun(run,{x:1,y:0},1/60,{});assert.ok(run.electricalControlAuthority<1);
...
test('Electrical visuals and developer map derive from the shared effective-intensity authority',async()=>{
  const rendererSource=await readFile(new URL('../src/veil/renderer.js',import.meta.url),'utf8');
  assert.match(rendererSource,/electricalEffectiveAt\(sample,run\.map\.worldState,\{shockStructures:run\.map\.shockStructures\}\)/);
  assert.match(rendererSource,/run\.time.*sample\.phase/s);
