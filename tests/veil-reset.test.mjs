...
function fixture(){
 ...
 const resources=createResources({storage});resources.collect(30,95);resources.learn('hydrogen');resources.makeHydrogen(3);resources.state.progress.cleared=true;resources.state.progress.runs=7;resources.state.progress.special='pure-h';
...
for(let fail=0;fail<=5;fail++){
 ...
 const reloaded=createResources({storage:f.storage});assert.equal(reloaded.blocked,false);assert.equal(reloaded.state.elements.H,0);assert.equal(reloaded.state.molecules.hydrogen,0);assert.deepEqual(reloaded.state.recipes,[]);assert.equal(reloaded.state.workspace,null);assert.equal(reloaded.state.progress.cleared,false);assert.equal(reloaded.state.progress.runs,0);assert.equal(reloaded.state.progress.bestChain,0);assert.equal(reloaded.state.pendingReset,undefined);
...
for(const category of RESET_CATEGORIES){
 ...
 if(category==='molecules'){assert.equal(r.state.molecules.hydrogen,0);assert.deepEqual(r.state.workspace,before.workspace);}
