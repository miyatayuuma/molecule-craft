...
try{
  ...
  await send('Runtime.enable');await send('Page.enable');await viewport(390,844,true);await send('Page.navigate',{url:`${origin}/`});
  ...
  await select('propellant','06-craft-neutral.png');
  const neutral=await evaluate(`(()=>{const rect=node=>{const r=node?.getBoundingClientRect();return r&&{left:r.left,top:r.top,right:r.right,bottom:r.bottom,width:r.width,height:r.height,x:r.left+r.width/2,y:r.top+r.height/2};};const map=document.querySelector('#supply-dialog .collector-shell-map');return{handle:rect(document.querySelector('#collector-launch-handle')),craft:rect(map.querySelector('.loadout-craft-image')),pulse:rect(map.querySelector('.loadout-pulse-image')),shock:rect(map.querySelector('.loadout-shock-image')),drive:rect(map.querySelector('.loadout-drive-image'))};})()`);
  assert.ok(neutral.handle.left<neutral.craft.right&&neutral.handle.right>neutral.craft.left&&neutral.handle.top<neutral.craft.bottom&&neutral.handle.bottom>neutral.craft.top,'CRAFT drag hit area must overlap the visible CRAFT body');
  await screenshot('06-craft-neutral.png');

  const handle=neutral.handle;await send('Input.dispatchMouseEvent',{type:'mousePressed',x:handle.x,y:handle.y,button:'left',buttons:1,clickCount:1});await waitFor("document.querySelector('#expedition-destinations')?.getAttribute('aria-hidden')==='false'",'CRAFT drag did not open destination layer');
  const target=await point("#expedition-destinations button[data-region='carbon']");assert.ok(target,'CRAFT drag destination missing');
  await send('Input.dispatchMouseEvent',{type:'mouseMoved',x:target.x,y:target.y,button:'left',buttons:1});await pause(120);
  const dragging=await evaluate(`(()=>{const rect=node=>{const r=node?.getBoundingClientRect();return r&&{left:r.left,top:r.top,right:r.right,bottom:r.bottom,width:r.width,height:r.height,x:r.left+r.width/2,y:r.top+r.height/2};};const map=document.querySelector('#supply-dialog .collector-shell-map');return{craft:rect(map.querySelector('.loadout-craft-image')),pulse:rect(map.querySelector('.loadout-pulse-image')),shock:rect(map.querySelector('.loadout-shock-image')),drive:rect(map.querySelector('.loadout-drive-image'))};})()`);
  assert.ok(Math.hypot(dragging.craft.x-neutral.craft.x,dragging.craft.y-neutral.craft.y)>2,'CRAFT body must move during launch drag');
  for(const key of ['pulse','shock','drive'])assert.ok(Math.abs(dragging[key].x-neutral[key].x)<=1&&Math.abs(dragging[key].y-neutral[key].y)<=1,`${key} must remain fixed during CRAFT drag`);
  await screenshot('07-craft-drag.png');
  await send('Input.dispatchMouseEvent',{type:'mouseReleased',x:target.x,y:target.y,button:'left',buttons:0,clickCount:1});
  console.log('LOADOUT optical acceptance passed:',JSON.stringify({pulse,shock,fuel,oxygen,coolant,neutral,dragging,captureEnabled}));
}finally{
