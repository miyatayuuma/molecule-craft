...
try{
  ...
  const desktopStart=await point('#collector-launch-handle');assert.ok(desktopStart?.width>0&&desktopStart?.height>0,'desktop CRAFT drag handle is not measurable');const desktopHit=await evaluate(`(()=>{const h=document.querySelector('#collector-launch-handle'),r=h?.getBoundingClientRect(),hit=r?document.elementFromPoint(r.left+r.width/2,r.top+r.height/2):null;window.__loadoutCraftPointerProbe=null;h?.addEventListener('pointerdown',event=>{window.__loadoutCraftPointerProbe={button:event.button,pointerId:event.pointerId,clientX:event.clientX,clientY:event.clientY};},{capture:true,once:true});return {rect:r&&{left:r.left,top:r.top,width:r.width,height:r.height},hitId:hit?.id??'',hitClass:hit?.className??'',pointerEvents:h?getComputedStyle(h).pointerEvents:''};})()`);assert.equal(desktopHit.hitId,'collector-launch-handle',`desktop CRAFT handle is not topmost at its center: ${JSON.stringify(desktopHit)}`);await send('Input.dispatchMouseEvent',{type:'mouseMoved',x:desktopStart.x,y:desktopStart.y,button:'none',buttons:0});await send('Input.dispatchMouseEvent',{type:'mousePressed',x:desktopStart.x,y:desktopStart.y,button:'left',buttons:1,clickCount:1});await pause(240);const desktopLaunchState=await evaluate(`(()=>({open:document.querySelector('#expedition-destinations')?.getAttribute('aria-hidden')==='false',probe:window.__loadoutCraftPointerProbe,expanded:document.querySelector('#collector-launch-handle')?.getAttribute('aria-expanded')}))()`);assert.equal(desktopLaunchState.open,true,`desktop CRAFT drag did not open destination layer: ${JSON.stringify({desktopHit,desktopLaunchState})}`);const desktopDragPoint={x:desktopStart.x+18,y:desktopStart.y+8};await send('Input.dispatchMouseEvent',{type:'mouseMoved',x:desktopDragPoint.x,y:desktopDragPoint.y,button:'left',buttons:1});await pause(120);const desktopDragging=await inspectCraft();assertCraftVisible(desktopDragging);assert.ok(Math.hypot(desktopDragging.flight.x-desktopDocked.flight.x,desktopDragging.flight.y-desktopDocked.flight.y)>2,'desktop flight craft must move');assertStationFixed(desktopDocked,desktopDragging,'desktop CRAFT drag');await screenshot('12-craft-desktop-drag.png');await send('Input.dispatchMouseEvent',{type:'mouseReleased',x:desktopDragPoint.x,y:desktopDragPoint.y,button:'left',buttons:0,clickCount:1});await waitFor("document.querySelector('#expedition-destinations')?.getAttribute('aria-hidden')==='true",'desktop CRAFT drag did not reset after cancel');
