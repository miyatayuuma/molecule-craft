...
test('bindCraftControls exposes Undo beside a trash-can full-cleanup control without changing hold safety',()=>{
  ...
  const undo=node('undo-cleanup'),clear=node('clear-all'),frame=node('frame-structure'),del=node('delete-selected'),actions=node('actions'),palette=node('palette'),focus=node('structure-focus'),viewer=node('viewer'),canvas=node('canvas'),help=node('help');
  help.textContent='片付けるは1秒長押し。図鑑の発見は残ります。';actions.insertBefore=(child,before)=>{actions.inserted=[child,before];};
  const document={hidden:false,defaultView:{addEventListener(){}},querySelector(selector){if(selector==='.viewer-actions')return actions;if(selector==='#undo-cleanup')return undo;if(selector==='#clear-all')return clear;if(selector==='#frame-structure')return frame;if(selector==='#delete-selected')return del;return null;},querySelectorAll(selector){return selector==='#help-dialog .help-steps p'?[help]:[];},addEventListener(){}};
  for(const item of [undo,clear,frame,del,actions,palette,focus,viewer,canvas,help])item.ownerDocument=document;
