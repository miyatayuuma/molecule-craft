...
function node(id=''){
  return{id,hidden:id==='undo-cleanup',disabled:false,className:'',textContent:id==='undo-cleanup'?'整理を戻す':'',attributes:new Map(),listeners:new Map(),style:{setProperty(){}},setAttribute(k,v){this.attributes.set(k,String(v));},getAttribute(k){return this.attributes.get(k)??null;},addEventListener(k,fn){this.listeners.set(k,fn);},querySelectorAll(){return[];}};
}
...
test('bindCraftControls exposes Undo beside a trash-can full-cleanup control without changing hold safety',()=>{
  ...
  const nodes=new Map([['undo-cleanup',undo],['clear-all',clear],['frame-structure',frame],['delete-selected',del]]);
  const document={querySelector(selector){if(selector==='.viewer-actions')return actions;if(selector==='#undo-cleanup')return undo;if(selector==='#clear-all')return clear;if(selector==='#frame-structure')return frame;if(selector==='#delete-selected')return del;return null;},addEventListener(){}};
  ...
  assert.ok(clear.listeners.has('pointerdown')||clear.listeners.has('click'),'Full cleanup remains bound through the existing hold action');
});
