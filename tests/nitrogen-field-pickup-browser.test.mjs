...
try{
  ...
  await waitFor(`globalThis.__nitrogenPickupRun?.map?.signals?.some(s=>s.nitrogenCritical===true&&s.claimable===true)`,'actual current-run N pickup did not make the N2 marker claimable after engagement',80);
  const marker=await evaluate(`(()=>{const s=globalThis.__nitrogenPickupRun.map.signals.find(s=>s.nitrogenCritical===true&&s.claimable===true);return {claimable:s?.claimable??false,ready:s?.ready??false};})()`);assert.deepEqual(marker,{claimable:true,ready:false});
