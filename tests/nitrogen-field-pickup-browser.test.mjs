...
try{
  ...
  await evaluate(`(()=>{const run=globalThis.__nitrogenPickupRun,site=run.map.safeExtractionSite;Object.assign(run.player,{x:site.x,y:site.y,vx:0,vy:0,speed:0,boost:0,combustion:false});run.driveHeld=false;})()`);
  await waitFor(`document.querySelector('#veil-return')?.dataset.returnState==='site-ready'`,'Safe Extraction Site did not enable the explicit Nitrogen return');await evaluate(`document.querySelector('#veil-return').click()`);
