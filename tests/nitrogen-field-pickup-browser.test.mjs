...
try{
  ...
  await waitFor(`globalThis.__nitrogenPickupRun?.map?.signals?.find(s=>s.nitrogenCritical===true)?.claimable===true`,'actual current-run N pickup did not make the N2 marker claimable after engagement',80);
  const marker=await evaluate(`(()=>{const s=globalThis.__nitrogenPickupRun.map.signals.find(s=>s.nitrogenCritical===true);return {claimable:s.claimable,ready:s.ready};})()`);assert.deepEqual(marker,{claimable:true,ready:false});
