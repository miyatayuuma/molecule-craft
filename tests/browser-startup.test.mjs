...
try{
  ...
  for(let attempt=0;attempt<80;attempt++){
    ...
    try{
      ...
      if(snapshot?.body?.includes('図鑑を読み込めませんでした。原子からの制作は続けられます。')||snapshot?.text?.includes('0/135'))break;
    }catch{}
  }
  ...
  assert.match(snapshot.text,/図鑑\s*0\/135/,'Encyclopedia startup denominator must resolve to the 135-molecule production DB');
...
console.log('Browser startup passed: Encyclopedia initializes against the 135-molecule production catalog without module parse failures.');
