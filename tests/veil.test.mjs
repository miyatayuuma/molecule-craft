...
assert.equal(r.state.elements.H,0);assert.equal(r.fillTankFromElements('propellant','hydrogen',1),false);
...
r.collect(20,25);r.learn('hydrogen');assert.equal(r.fillTankFromElements('propellant','hydrogen',1).added,1);r.save();
assert.equal(r.state.elements.H,18);assert.deepEqual(r.state.tanks.propellant,{molecule:'hydrogen',amount:1});
const drive=createResources({storage:memory()});drive.collect(240);drive.learn('hydrogen');assert.equal(drive.fillTankFromElements('propellant','hydrogen',120).added,120);assert.equal(drive.consumeBoost(),true);assert.deepEqual(drive.state.tanks.propellant,{molecule:'hydrogen',amount:80});
assert.equal(r.fillTankFromElements('propellant','hydrogen',5).added,5);assert.equal(r.state.elements.H,8);assert.equal(r.fillTankFromElements('propellant','hydrogen',5),false);
assert.equal(r.spend({H:9,C:1}),false);assert.equal(r.state.elements.H,8,'Rejected parts must not partially spend');
r.save();const reboot=createResources({storage});assert.equal(reboot.state.elements.H,8);assert.deepEqual(reboot.state.tanks.propellant,{molecule:'hydrogen',amount:6});assert.equal(reboot.state.progress.bestChain,25);
...
assert.ok(gap()>0);
function gate(boost){const run=createRun(createMap(2),VEIL,{fuel:{hydrogen:40},predators:false});Object.assign(run.player,{x:VEIL.gate.x,y:VEIL.gate.y+130,angle:-Math.PI/2,speed:VEIL.speed});if(boost)beginBurst(run,()=>true);for(let i=0;i<120;i++)stepRun(run,{x:0,y:-1},1/60);return run;}
