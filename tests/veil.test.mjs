...
const drive=createResources({storage:memory()});drive.learn('hydrogen');drive.state.molecules.hydrogen=120;assert.equal(drive.fillTank('hydrogen'),true);assert.equal(drive.consumeBoost(),true);assert.equal(drive.state.molecules.hydrogen,0);assert.deepEqual(drive.state.tanks.propellant,{molecule:'hydrogen',amount:80});
