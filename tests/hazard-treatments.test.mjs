...
test('treatment authority uses only the three approved Utility molecules and exact two-molecule batch costs',()=>{
  assert.deepEqual(HAZARD_TREATMENT_IDS,['mechanical','abrasive','thermal']);
...
test('failed or incomplete treatment transaction never partially consumes BASE stock and states stay independent',()=>{
  const r=createResources({storage:null});r.setCatalog(catalog);awakened(r);for(const id of ['phosphoric-acid','sulfuric-acid','difluoromethane'])r.discover(id);
  Object.assign(r.state.elements,{H:50,C:20,O:50,P:8,S:8,F:3});const before={...r.state.elements};
  ...
  assert.equal(r.applyHazardTreatment('abrasive').committed,true);assert.equal(r.state.treatments.mechanical,1);assert.equal(r.state.treatments.abrasive,1);
});

test('depletion uses resolved pre-mitigation effective intensity, max-overlap by type, and ignores wrong hazards',()=>{
  const mechanical=defineHazard('test-mech',HAZARD_TYPES.MECHANICAL,'pressure'),thermal=defineHazard('test-thermal',HAZARD_TYPES.THERMAL,'hot-zone');
  const treatments={mechanical:1,abrasive:1,thermal:1},resolved=createHazardTreatmentExposureState(),hazards=[
    hazardSample(mechanical,1,{effectiveIntensity:1.25}),hazardSample(mechanical,.8,{effectiveIntensity:.8}),hazardSample(thermal,.5,{effectiveIntensity:.5}),
  ];
  updateHazardTreatmentExposure(treatments,hazards,10,resolved);assert.equal(resolved.mechanical,1.25);assert.equal(resolved.thermal,.5);assert.ok(Math.abs(treatments.mechanical-.875)<1e-10);assert.ok(Math.abs(treatments.thermal-.95)<1e-10);assert.equal(treatments.abrasive,1);
...
test('treatment charge persists across settlement/reload including forced return and legacy v8 saves normalize safely',()=>{
  ...
  const legacy=createInitialResourcesState();delete legacy.treatments;let raw=JSON.stringify(legacy);const legacyStore={getItem:()=>raw,setItem:(_k,v)=>{raw=String(v);},removeItem:()=>{}};const normalized=createResources({storage:legacyStore});assert.deepEqual(normalized.state.treatments,{mechanical:0,abrasive:0,thermal:0});
});
...
test('Rare ecology suppression follows treatment-equivalent reserve and recovers after DOCK spend while Cl remains independent',()=>{
  ...
  assert.equal(rareEcologyTreatmentReserve('P',6),3);assert.equal(rareEcologyTreatmentReserve('S',6),3);assert.equal(rareEcologyTreatmentReserve('F',12),3);assert.equal(rareEcologyTreatmentReserve('Cl',12),null);
