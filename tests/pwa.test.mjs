...
const network=good.network;response=await good.call('fetch',{request:new Request(good.scope+'src/app-v14.js?v=32')});assert.match(await response.text(),/saveWorkspace/);assert.equal(good.network,network,'Cached release must serve without network');
