...
try{
  ...
  await evaluate(`document.querySelector('#launch-veil').click()`);await waitFor(`document.querySelector('#veil-view')?.hidden===false&&!!globalThis.__rareSurveyRun`,'Rare Survey expedition did not launch');exceptions.length=0;

  const visual=await evaluate(`(()=>{const run=globalThis.__rareSurveyRun,site=run.rareSurvey?.anomalies?.find(item=>item.element==='P'),dust=site&&run.map.dust.find(item=>item.rareAnomaly===site.id);if(!site||!dust)return null;run.predators=false;Object.assign(run.player,{x:site.x,y:site.y+150,vx:0,vy:0,speed:0,angle:-Math.PI/2});const renderer=globalThis.__rareSurveyRenderer,canvas=document.querySelector('#veil-canvas'),ctx=canvas.getContext('2d');renderer.reset();renderer.draw(run,0,false);const rect=canvas.getBoundingClientRect(),sx=canvas.width/rect.width,sy=canvas.height/rect.height,q=renderer.screen(dust.x,dust.y),cx=Math.round(q.x*sx),cy=Math.round(q.y*sy),radius=Math.max(3,Math.round(5*Math.max(sx,sy))),pixels=ctx.getImageData(Math.max(0,cx-radius),Math.max(0,cy-radius),Math.min(canvas.width,cx+radius+1)-Math.max(0,cx-radius),Math.min(canvas.height,cy+radius+1)-Math.max(0,cy-radius)).data;let best=[0,0,0],score=-1;for(let i=0;i<pixels.length;i+=4){const next=pixels[i]+pixels[i+1]+pixels[i+2];if(pixels[i+3]>0&&next>score){score=next;best=[pixels[i],pixels[i+1],pixels[i+2]];}}return {id:site.id,x:site.x,y:site.y,kind:dust.kind,element:dust.element,color:best};})()`);
  assert.ok(visual);assert.equal(visual.kind,'rare');assert.equal(visual.element,'P');assert.ok(visual.color[0]>visual.color[2]+25,`Rare anomaly must render as a warm gold object rather than ordinary dust: ${JSON.stringify(visual.color)}`);
