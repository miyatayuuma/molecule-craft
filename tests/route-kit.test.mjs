...
const properIntersection=(a,b,c,d)=>{
  const abC=cross(a,b,c),abD=cross(a,b,d),cdA=cross(c,d,a),cdB=cross(c,d,b),epsilon=1e-7;
  return abC*abD<-epsilon&&cdA*cdB<-epsilon;
};
function assertSimplePath(points,label){
  for(let i=0;i<points.length-1;i++)for(let j=i+3;j<points.length-1;j++){
    assert.equal(properIntersection(points[i],points[i+1],points[j],points[j+1]),false,`${label} crosses itself at segments ${i}/${j}`);
  }
}
...
assertSimplePath(OXYGEN_VORTEX_ROUTE.points,'O₂ vortex centerline');
for(const [index,line]of OXYGEN_VORTEX_ROUTE.fieldLines.entries())assertSimplePath(line,`O₂ vortex field line ${index}`);
...
console.log('Route Kit passed: reusable connected primitives, sockets, non-self-crossing vortex geometry and shared guidance flow.');
