...
test('Current FIELD developer map exposes Nitrogen v3 geometry without implying a player-facing belt',()=>{
  ...
  assert.match(svg,new RegExp(`data-route="${NITROGEN_SIDE_ROUTE.id}" data-optional="true"`));
  assert.match(svg,new RegExp(`data-resource-area="high-density-pocket"[\\s\\S]*cx="${fmt(NITROGEN_HIGH_DENSITY_POCKET.x)}" cy="${fmt(NITROGEN_HIGH_DENSITY_POCKET.y)}"`));
  for(const group of NITROGEN_RESOURCE_GROUPS)assert.match(svg,new RegExp(`data-resource-group="${group.id}" data-particles="${group.particles}"`));
  for(const area of NITROGEN_RECOVERY_AREAS)assert.match(svg,new RegExp(`data-recovery-area="${area.id}"[\\s\\S]*data-safe-extraction="${area.safeExtraction===true}"`));
  assert.match(svg,new RegExp(`data-nitrogen-pulse-lip="${NITROGEN_PULSE_FIELD.id}"[\\s\\S]*r="${fmt(NITROGEN_PULSE_FIELD.radius)}"`));assert.match(svg,new RegExp(`data-critical-insight-area="${NITROGEN_INSIGHT_AREA.id}"`));
  for(const anchor of NITROGEN_INSIGHT_ANCHORS)assert.match(svg,new RegExp(`data-resonance-anchor="${anchor.id}"`));
  assert.doesNotMatch(svg,/data-rare-anomaly=/,'retired Rare Survey markers must not appear on the developer map');assert.match(svg,new RegExp(`data-nitrogen-core="${NITROGEN_CORE.id}"[\\s\\S]*data-body-radius="${fmt(NITROGEN_CORE.radius)}"[\\s\\S]*data-fracture-radius="${fmt(NITROGEN_CORE.fractureRadius)}"`));
});
