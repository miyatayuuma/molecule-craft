...
import {NITROGEN_CORE,NITROGEN_HAZARDS,NITROGEN_HIGH_DENSITY_POCKET,NITROGEN_INSIGHT_AREA,NITROGEN_RARE_CL_SITE,NITROGEN_RECOVERY_AREAS,NITROGEN_ROUTE,NITROGEN_ZONES} from '../src/veil/nitrogen-routes.js';
...
test('Current FIELD developer map exposes Nitrogen v2 geometry without implying a player-facing belt',()=>{
  ...
  assert.match(svg,new RegExp(`data-resource-area="high-density-pocket"[\\s\\S]*cx="${fmt(NITROGEN_HIGH_DENSITY_POCKET.x)}" cy="${fmt(NITROGEN_HIGH_DENSITY_POCKET.y)}"`));for(const area of NITROGEN_RECOVERY_AREAS)assert.match(svg,new RegExp(`data-recovery-area="${area.id}"`));assert.match(svg,new RegExp(`data-critical-insight-area="${NITROGEN_INSIGHT_AREA.id}"`));assert.match(svg,new RegExp(`data-rare-anomaly="${NITROGEN_RARE_CL_SITE.id}" data-element="Cl"`));assert.match(svg,new RegExp(`data-nitrogen-core="${NITROGEN_CORE.id}"[\\s\\S]*data-fracture-radius="${fmt(NITROGEN_CORE.fractureRadius)}"`));
});
