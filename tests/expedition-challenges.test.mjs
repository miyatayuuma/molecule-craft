...
const heatZone=EXPEDITION_CHALLENGES.find(challenge=>challenge.id==='thermal'),heatPoint={x:challengeCenter(heatZone,heatZone.centerY),y:heatZone.centerY};
assert.equal(challengeEnvironment(heatPoint,0).heat,48);
assert.equal(environmentAt(heatPoint,0).heat,48);
