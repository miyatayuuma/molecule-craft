...
for(const zone of [pulse,curve,thermal])assert.equal(Object.hasOwn(zone,'rewards'),false,'challenge geometry must not own fixed molecule rewards');
...
for(const [zone,route] of [[pulse,network['oxygen-shortcut']],[curve,deep['oxygen-deep-skill']],[thermal,deep['oxygen-deep-thermal']]]){
  ...
  assert.deepEqual(run.events,[],`${zone.id} correct-route completion must leave molecule insight ownership to the Graph frontier`);
