...
try{
  ...
  await waitFor("!!document.querySelector('#open-reaction-lab')&&!document.querySelector('#open-dock')",'LAB did not initialize');
  ...
  const before=await evaluate(`(()=>{const s=JSON.parse(localStorage.getItem('molecule-craft.resources.v1'));return JSON.stringify({elements:s.elements,tanks:s.tanks,recipes:s.recipes,hints:s.hints,progress:s.progress});})()`);
  assert.match(initial.text,/REACTIONS OFF/);assert.equal(await evaluate("document.querySelector('#open-reaction-lab').previousElementSibling?.id"),'open-supply','LAB is placed beside LOADOUT after legacy DOCK removal');assert.ok(initial.rect.top>=0&&initial.rect.bottom<=844&&initial.rect.left>=0&&initial.rect.right<=390,'Reaction Lab must fit the mobile viewport');
  ...
  const after=await evaluate(`(()=>{const s=JSON.parse(localStorage.getItem('molecule-craft.resources.v1'));return JSON.stringify({elements:s.elements,tanks:s.tanks,recipes:s.recipes,hints:s.hints,progress:s.progress});})()`);assert.equal(after,before,'interaction prototype must not consume stock or mutate progression');
