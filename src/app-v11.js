...
function bindUI() {
  document.querySelector('#delete-selected')?.addEventListener('click', () => {
    ...
    selectedAtomId = null; activeTorsionKey = null;
  ...
  c.addEventListener('wheel', e => {
    e.preventDefault();
    if (relaxation) { pulse('構造安定化中はカメラを固定しています'); return; }
    zoomCamera(Math.exp(e.deltaY * 0.001));
  }, { passive:false });
...
function onPointerDown(e) {
  if (relaxation) { pulse('安定構造へ移動中 · カメラと構造操作を固定しています'); return; }
  ...
  if (bondHit) {
    ...
    bondHoldTimer = setTimeout(() => {
      ...
      bondHoldInterval = setInterval(() => weakenBond(key), 520);
    }, 560);
    capture(e); return;
  }
...
function onPointerMove(e) {
  ...
  if (dragState.mode === 'torsion') {
    rotateAroundBond(dragState.bond, dragState.ids, (dx-dy*.25)*.012);
    relaxGeometryStep(.20, 1);
  }
  if (dragState.mode === 'structure') {
    rotateBranchTowardScreen(dragState.pivotId, dragState.atomId, dragState.ids, dx, dy);
    relaxGeometryStep(.18, 1);
  }
  renderMolecule();
}
...
function onPointerUp(e) {
  ...
  if (state.mode === 'bond') {
    ...
    if (!isTap) startRelaxation('ねじり後の構造を安定化しています', {maxDuration:1050});
  } else if (state.mode === 'camera') {
    if (isTap) { selectedAtomId = null; activeTorsionKey = null; }
  }
  dragState = null; release(e);
  if (!relaxation) refresh();
}
...
function strengthenBond(key) {
  ...
  startRelaxation(old===1?'二重結合へ変化 · sp²配置へ移動中':'三重結合へ変化 · sp配置へ移動中', {maxDuration:1450});
}
...
function weakenBond(key) {
  ...
  activeTorsionKey=null;
  startRelaxation(old===3?'二重結合へ戻りながら安定化中':old===2?'単結合へ戻りながら安定化中':'結合解除後の構造を安定化中', {maxDuration:1250});
}
...
function electronDrop(sourceId,e) {
  ...
  if (existing) {
    ...
    message=old===1?'電子を共有 · 二重結合の平面構造へ移動中':'電子を共有 · 三重結合の直線構造へ移動中';
  } else {
    ...
    message=sameComponent?'環を閉じながら安定構造へ移動中':'単結合を作成 · 安定構造へ移動中';
  }
  selectedAtomId=targetId;
  startRelaxation(message, {maxDuration:1500});
}
...
function startRelaxation(message='安定構造へ移動中', options={}) {
  ...
  if (reduceMotion) {
    for(let i=0;i<90;i++) relaxGeometryStep(1, 1);
    stopRelaxation(); refresh(); return;
  }
  relaxation = {
    startedAt: performance.now(),
    minDuration: options.minDuration ?? 520,
    maxDuration: options.maxDuration ?? 1350,
    stableFrames: 0,
    message,
  };
  selectionChip.textContent = `${message} · カメラ固定`;
  renderMolecule();
  refreshInfo(true);
}
...
function updateRelaxation(now) {
  ...
  const elapsed = now-relaxation.startedAt;
  const strengthScale = elapsed < 280 ? .72 : elapsed < 700 ? .52 : .34;
  const maxMove = relaxGeometryStep(strengthScale, 2);
  renderMolecule();
  selectionChip.textContent = `${relaxation.message} · カメラ固定`;
  if (maxMove < .0018) relaxation.stableFrames += 1; else relaxation.stableFrames = 0;
  if ((elapsed >= relaxation.minDuration && relaxation.stableFrames >= 7) || elapsed >= relaxation.maxDuration) {
    relaxation = null;
    refresh();
  }
}
...
function relaxGeometryStep(scale=.5, passes=1){
  ...
  for(let pass=0;pass<passes;pass++){
    const before=new Map(molecule.atoms.map(a=>[a.id,pos(a.id)?.clone()]));
    const aromatic=findCycles(8).filter(isAromaticSixCarbonCycle);
    for(const b of molecule.bonds){
      const a=pos(b.a),c=pos(b.b); if(!a||!c)continue;
      const d=c.clone().sub(a),len=Math.max(.001,d.length()),target=bondLengthFor(b.a,b.b,b.order);
      const corr=d.normalize().multiplyScalar((len-target)*.065*scale);
    ...
    for(const atom of molecule.atoms){
      const p=pos(atom.id),old=before.get(atom.id);if(p&&old)maxMove=Math.max(maxMove,p.distanceTo(old));
    }
  }
  return maxMove;
}
...
function relaxCenter(id,strength){
  ...
  for(let i=0;i<ids.length;i++)for(let j=i+1;j<ids.length;j++){
    ...
    if(ti.lengthSq()>1e-8){vi.addScaledVector(ti.normalize(),-err*strength).normalize();pi.lerp(c.clone().addScaledVector(vi,li),.32);}
    if(tj.lengthSq()>1e-8){vj.addScaledVector(tj.normalize(),-err*strength).normalize();pj.lerp(c.clone().addScaledVector(vj,lj),.32);}
  }
}
...
function enforceAngle(c,aId,bId,target,strength){
  ...
  a.lerp(c.clone().addScaledVector(va,la),.34);b.lerp(c.clone().addScaledVector(vb,lb),.34);
}
...
function refreshInfo(keepRelaxationMessage=false){
  ...
  if(!selected){selectedElementEl.textContent=selectedValenceEl.textContent=selectedLimitEl.textContent='—';if(!keepRelaxationMessage)selectionChip.textContent=molecule.atoms.length?'背景ドラッグで回転 · 原子を選択して追加':'元素を押して中心原子を置く';return;}
  ...
  if(!keepRelaxationMessage)selectionChip.textContent=activeTorsionKey?'回転軸固定中 · 原子ドラッグでねじる':`${selected.element} 選択中 · 原子ドラッグで構造調整`;
}
...
function pulse(text){selectionChip.textContent=text;clearTimeout(pulse.t);pulse.t=setTimeout(()=>{if(relaxation)selectionChip.textContent=`${relaxation.message} · カメラ固定`;else refreshInfo();},1700);}
...
function updateTwoFinger(){
  if(relaxation||activePointers.size!==2||!multiGesture)return;
...
function animate(now=performance.now()){
  ...
  if(relaxation)updateRelaxation(now);
  camera.lookAt(cameraTarget);camera.updateMatrixWorld();renderer.render(scene,camera);
}
