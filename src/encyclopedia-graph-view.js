...
const GRAPH_MOTION_CHEVRON_CLEARANCE=4;
...
function updateGraphChevrons(records,blockingCircles){
...
export function renderEncyclopediaGraph({
  ...
  const nodeCircles=()=>[...interactiveNodeById.values()].map(node=>graphNodeCircleInSvg(node,svg,width,height)).filter(Boolean),motionNodeCircles=edgeProgress=>{const settle=clamp(((edgeProgress??0)-.9)/.1,0,1),extra=GRAPH_MOTION_CHEVRON_CLEARANCE*(1-settle);return nodeCircles().map(circle=>({...circle,radius:circle.radius+extra}));};if(!reduceMotion&&!suppressMotion){for(const record of chevronEdges)record.chevron?.setAttribute('visibility','hidden');runGraphGeometryMotion(graphMotion,geometryTweens,edgeTweens,chevronEdges,{win,nodeCircles:motionNodeCircles});}else updateGraphChevrons(chevronEdges,nodeCircles());
