...
export function renderEncyclopediaGraph({
  ...
  const nodeCircles=()=>[...stage.querySelectorAll('.graph-node')].map(node=>graphNodeCircleInSvg(node,svg,width,height)).filter(Boolean);if(!reduceMotion&&!suppressMotion){updateGraphChevrons(chevronEdges,nodeCircles());runGraphGeometryMotion(graphMotion,geometryTweens,edgeTweens,chevronEdges,{win,nodeCircles});}else updateGraphChevrons(chevronEdges,nodeCircles());
