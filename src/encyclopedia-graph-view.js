...
export function renderEncyclopediaGraph({
  ...
  const nodeCircles=()=>[...interactiveNodeById.values()].map(node=>graphNodeCircleInSvg(node,svg,width,height)).filter(Boolean),motionNodeCircles=edgeProgress=>{const extra=GRAPH_MOTION_CHEVRON_CLEARANCE*(1-clamp(edgeProgress??0,0,1));return nodeCircles().map(circle=>({...circle,radius:circle.radius+extra}));};if(!reduceMotion&&!suppressMotion){for(const record of chevronEdges)record.chevron?.setAttribute('visibility','hidden');runGraphGeometryMotion(graphMotion,geometryTweens,edgeTweens,chevronEdges,{win,nodeCircles:motionNodeCircles});}else updateGraphChevrons(chevronEdges,nodeCircles());
