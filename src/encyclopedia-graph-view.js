...
function runGraphGeometryMotion(state,tweens,edgeTweens,chevronRecords,{duration=ENCYCLOPEDIA_MOTION.graphNavigationDuration,edgeDelay=ENCYCLOPEDIA_MOTION.graphEdgeDelay,nodeCircles=null,win=globalThis.window}={}){
  ...
  const step=()=>{
    ...
    updateGraphChevrons(chevronRecords,nodeCircles?.()??null);
...
export function renderEncyclopediaGraph({
  ...
  const nodeCircles=()=>[...interactiveNodeById.values()].map(node=>graphNodeCircleInSvg(node,svg,width,height)).filter(Boolean),motionNodeCircles=()=>nodeCircles().map(circle=>({...circle,radius:circle.radius+GRAPH_MOTION_CHEVRON_CLEARANCE}));if(!reduceMotion&&!suppressMotion){for(const record of chevronEdges)record.chevron?.setAttribute('visibility','hidden');runGraphGeometryMotion(graphMotion,geometryTweens,edgeTweens,chevronEdges,{win,nodeCircles:motionNodeCircles});}else updateGraphChevrons(chevronEdges,nodeCircles());
