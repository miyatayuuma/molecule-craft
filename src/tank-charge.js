...
function drawStage(stage,{record,use,progress,fillProgress,oldRatio,newRatio,replacing,reduced,visualCycles=1,fade=1},metrics){
  ...
  if(atoms.length&&fillProgress>0){
    if(phase<.42){
      const t=phase/.42;
      for(let i=0;i<Math.min(10,atoms.length);i++){const angle=i*2.399+phase*9.5,radius=18-(t*7)+(i%3)*2,x=center.x+Math.cos(angle)*radius,y=center.y+Math.sin(angle)*radius*.65;ctx.fillStyle=ELEMENTS[atoms[i]]?.color??'#d9f7ff';ctx.globalAlpha=(.55+.4*t)*fade;ctx.beginPath();ctx.arc(x,y,2.5+(atoms[i]==='H'?0:1),0,Math.PI*2);ctx.fill();}
    }else if(phase<.72){
      const t=(phase-.42)/.30,eased=1-Math.pow(1-t,3);
      for(let i=0;i<Math.min(layout.length,10);i++){const p=layout[i],jitter=(1-eased)*6,x=center.x+p.x*eased+Math.sin(i*2.1+phase*16)*jitter,y=center.y+p.y*eased+Math.cos(i*1.7+phase*14)*jitter;ctx.fillStyle=ELEMENTS[atoms[i]]?.color??'#d9f7ff';ctx.globalAlpha=(.7+.3*eased)*fade;ctx.beginPath();ctx.arc(x,y,atoms[i]==='H'?3:4,0,Math.PI*2);ctx.fill();}
      if(t>.55)drawMolecule(ctx,record,layout,{cx:center.x,cy:center.y,scale:eased,alpha:(t-.55)/.45*.55*fade});
    }else if(phase<.84){
      const pulse=1+Math.sin((phase-.72)/.12*Math.PI)*.08;drawMolecule(ctx,record,layout,{cx:center.x,cy:center.y,scale:pulse,alpha:fade});
    }else{
      const t=(phase-.84)/.16,eased=t*t*(3-2*t),tankCenter={x:tank.x+tank.w/2,y:tank.y+tank.h*.5},cx=center.x+(tankCenter.x-center.x)*eased,cy=center.y+(tankCenter.y-center.y)*eased,scale=1-eased*.78;drawMolecule(ctx,record,layout,{cx,cy,scale,alpha:(1-eased*.78)*fade});
    }
  }
  ctx.globalAlpha=1;ctx.strokeStyle=color;ctx.globalAlpha=.28*fade;ctx.beginPath();ctx.arc(center.x,center.y,12+Math.sin(fillProgress*Math.PI*8)*2,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=1;
}
...
export function bindTankChargeAction(button,{stage,use,record,planFor,commit,onStart=()=>{},onFinish=()=>{},clock=()=>performance.now(),raf=requestAnimationFrame,cancelRaf=cancelAnimationFrame,reduced=globalThis.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches??false,transferEffects=null}={}){
  ...
  function fadeOut(){
    ...
    function drain(){if(!ownsStage())return;const progress=clamp((clock()-started)/320);if(progress>=1){hide();return;}if(drawing&&metrics)drawStage(stage,{...drawing,fillProgress:drawing.fillProgress+progress*.18,fade:1-progress},metrics);frame=raf(drain);}
  ...
  function tick(){
    if(!active)return;const elapsed=clock()-active.started,total=active.duration,raw=clamp(elapsed/total),afterGuard=clamp((elapsed-180)/Math.max(1,total-180)),fillProgress=active.plan.replacing?clamp((afterGuard-.3)/.7):afterGuard,projected=Math.floor(active.plan.maxAdd*fillProgress),oldRatio=active.plan.loadedCapacity?active.plan.amount/active.plan.loadedCapacity:0,newRatio=active.plan.replacing?projected/active.plan.capacity:(active.plan.current+projected)/active.plan.capacity,visualCycles=Math.min(4,Math.max(1,active.plan.maxAdd)),visualStep=fillProgress>0?Math.floor(Math.min(.9999,fillProgress)*visualCycles):-1;
