...
function drawStage(stage,{record,use,progress,fillProgress,oldRatio,newRatio,replacing,reduced,fade=1}){
  ...
  const atoms=record?.atoms??[];for(let i=0;i<Math.min(10,Math.max(4,atoms.length*2));i++){
    ...
    ctx.fillStyle=atomColor;ctx.globalAlpha=Math.sin(local*Math.PI)*fade;ctx.beginPath();ctx.arc(x,y,2.2+(i%3)*.45,0,Math.PI*2);ctx.fill();
  }
...
export function bindTankChargeAction(button,{stage,use,record,planFor,commit,onStart=()=>{},onFinish=()=>{},clock=()=>performance.now(),raf=requestAnimationFrame,cancelRaf=cancelAnimationFrame,reduced=globalThis.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches??false}={}){
  let active=null,frame=0,lastDrawing=null;const token={};const owner=button.ownerDocument,presentation=TANK_PRESENTATION[use];
  ...
  function fadeOut(){
    ...
    const started=clock(),drawing=lastDrawing;
    function drain(){if(!ownsStage())return;const progress=clamp((clock()-started)/320);if(progress>=1){hide();return;}if(drawing)drawStage(stage,{...drawing,fillProgress:drawing.fillProgress+progress*.18,fade:1-progress});frame=raf(drain);}
  ...
  function tick(){
    ...
    active.projected=projected;button.style.setProperty('--hold-progress',`${raw*100}%`);lastDrawing={record,use,progress:afterGuard,fillProgress,oldRatio,newRatio,replacing:active.plan.replacing,reduced};drawStage(stage,lastDrawing);if(raw>=1){finish(active.plan.maxAdd);return;}frame=raf(tick);
  }
  function begin(input){const plan=planFor();if(active||button.disabled||!plan?.maxAdd)return;onStart();cancelRaf(frame);stageOwners.set(stage,token);stage.classList.remove('fading');const fraction=plan.maxAdd/Math.max(1,plan.capacity),duration=plan.replacing?Math.max(600,450+1050*fraction):Math.max(360,1500*fraction);active={...input,started:clock(),duration,plan,projected:0};stage.hidden=false;stage.dataset.tankUse=use;stage.style.setProperty('--tank-color',presentation.color);stage.querySelector('#tank-charge-icon').textContent=presentation.icon;stage.querySelector('#tank-charge-label').textContent=`${plan.label}へ充填`;qResult(plan.replacing?'✕ → ◉':'● → ◉');stage.setAttribute('aria-label',plan.replacing?'旧内容を廃棄して入れ替え':'原子からタンクへ充填');stage.classList.add('charging');button.classList.add('holding');lastDrawing={record,use,progress:0,fillProgress:0,oldRatio:plan.loadedCapacity?plan.amount/plan.loadedCapacity:0,newRatio:plan.current/Math.max(1,plan.capacity),replacing:plan.replacing,reduced};drawStage(stage,lastDrawing);frame=raf(tick);}
