...
export function createResources({storage,onStatus=()=>{}}={}){
  ...
  const api={
    ...
    progressionInsightCandidates({cargo={},foundElements=[]}={}){if(blocked)return [];const found=new Set([...state.progress.foundElements,...foundElements]),eligible=[];if((state.elements.H??0)+(cargo.H??0)>=2)eligible.push('hydrogen');if(found.has('C'))eligible.push('methane');if(found.has('O'))eligible.push('oxygen','water');return eligible.filter(id=>records.has(id)&&!state.recipes.includes(id)&&!state.hints.includes(id));},
