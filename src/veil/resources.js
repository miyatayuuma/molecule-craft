...
const COLLECTION_KEY='molecule-craft.collection.v1',HELP_KEY='molecule-craft.help.v1',MANAGED=['H','C','O'],FREE=['N','Cl','S','P','F'],MAX=1e9;
...
function migrate(old){if(old.schemaVersion===2)return old;const next={...initialState(),...old,schemaVersion:2};next.elements={H:0,C:0,O:0,...old.elements};next.molecules={hydrogen:0,methane:0,oxygen:0,water:0,...old.molecules};next.progress={...initialProgress(),...old.progress};next.migrateDiscoveries=true;for(const a of next.workspace?.atoms??[])if(MANAGED.includes(a.element)&&!next.progress.foundElements.includes(a.element))next.progress.foundElements.push(a.element);return next;}
export function createResources({storage,onStatus=()=>{}}={}){
  ...
  const canAfford=cost=>!!cost&&Object.entries(cost).every(([s,n])=>integer(n)&&(MANAGED.includes(s)?state.elements[s]>=n:FREE.includes(s)));
  function spend(cost){if(blocked||!canAfford(cost))return false;for(const [s,n]of Object.entries(cost))if(MANAGED.includes(s))state.elements[s]-=n;return true;}
  function refund(cost){if(blocked)return;for(const [s,n]of Object.entries(cost))if(MANAGED.includes(s)&&integer(n))state.elements[s]=Math.min(MAX,state.elements[s]+n);}
