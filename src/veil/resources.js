...
export function createResources({storage,onStatus=()=>{}}={}){
  ...
  function fillEntriesAt(rate){
    ...
    for(const use of Object.keys(TANK_USES)){
      const molecule=selected[use],tank=state.tanks[use],valid=!molecule||state.recipes.includes(molecule)&&fitsTank(molecule,use)&&records.has(molecule),same=valid&&!!molecule&&tank.molecule===molecule,current=same?tank.amount:0,capacity=valid&&molecule?tankCapacity(use,molecule)??0:0;
  ...
  function candidateFeatures(candidate){
    ...
    const gameplayUtility=usesFor(candidate.id).length?1:0,cost=costFor(candidate.id),craftableNow=!!cost&&affordableCost(cost,state.elements);
