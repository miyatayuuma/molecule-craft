...
export function createResources({storage,onStatus=()=>{}}={}){
  ...
  function insightDestinationIds(){
    const ids=[...(state.progress.regions??[])].filter(id=>isExpeditionRegionDestination(id)&&hasInsightSitePool(id));
  ...
  api={
    ...
    settleExpedition(units,best=0,captured=false,{destinationReached=false,insights=[]}={}){
      ...
      const frontierInsight=finalizeFrontierRun(captured,insights,committedInsights,{seedBefore,seedAfter}),atoms={};for(const el of elements)atoms[el]=state.elements[el]-before[el];return {captured,rate,kept,lost,atoms,found,completedNow,committedInsights,frontierInsight};
    },
