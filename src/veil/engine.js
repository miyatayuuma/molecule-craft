...
export function moveFlight(p,input,dt,{config:c=VEIL,chain=0,assist=null,force={x:0,y:0}}={}){
  ...
  const target=boost?c.boostSpeed*corner:steering?c.speed*(1+fever*c.chainSpeedBonus)*magnitude*corner:c.driftSpeed;
  ...
  p.x=clamp(p.x+(p.vx+fx*resistance)*dt,c.bounds.left,c.bounds.right);
  p.y=clamp(p.y+(p.vy+fy*resistance)*dt,c.bounds.top,c.bounds.bottom);
...
export function createRun(map,config=VEIL){return {map,player:createFlight(config),time:0,chain:0,best:0,chainTime:0,collected:0,dustUnits:0,effects:[],events:[],denseUntil:0,gatePassed:false,departed:false,lap:false,laps:0,lastLap:0,config};}
...
function stepRunFrame(run,input,dt){
  ...
  const radius=c.suctionRadius+c.chainRadiusBonus*Math.min(run.chain/c.feverChain,1)+(boosted?c.boostRadius:0);
  ...
  if(picked)run.events.push({type:'pickup',amount:gained,chain:run.chain,count:picked});
