...
export function stepRun(run,input,elapsed,systems={}){
  ...
  const before=run.time,driveHeldBefore=!!run.driveHeld,events=stepBaseRun(run,input,elapsed,systems),simulated=Math.max(0,run.time-before),nitrogenSignal=events.find(event=>event.type==='signal'&&event.region==='nitrogen');
