...
import { createRun, stepRun, stepNormalExtractionPending, beginBurst, beginShock, setCombustionHeld, triggerInsight, discardActiveInsight, discardRunInsights, runInsightLossSnapshot } from './expedition-run.js';
...
export function createVeilUI({resources,canLeave=()=>true,canSupply=canLeave,onBeforeLaunch=()=>true,onCraft=()=>{},onCommit=()=>{}}){
  ...
  let renderer=null,run=null,lastTelemetry=null,active=false,paused=false,raf=0,last=0,hudAt=0,pointer=null,drivePointer=null,origin=null,shipTap=null,messageUntil=0,thermalNotice=null,thermalNoticeUntil=0,returnState=null,requestExpeditionLaunch=()=>false,launchTransaction=null;
  ...
  function updatePrompt(){
    const goal=run?.inspiration?{id:run.inspiration}:growthGoal(resources.state,{cargo:run?.collectedElements??{}}),record=goal.id?resources.record(goal.id):null;
  ...
  function returnMethod(){
    if(!active||!run||paused||run.captured||returnState?.phase!=='exploring')return null;
    if((run.carriedInsights?.length??0)>0)return 'insight';
    return isInsideSafeExtractionSite(run.player,run.map.safeExtractionSites)?'site':null;
  }
  ...
  function initializeExploreLaunch(prepared,nextRun){
    ...
    active=true;paused=false;returnState={phase:'exploring'};root.hidden=false;document.body.dataset.mode='veil';appShell.inert=true;
    ...
    const first=prepared.nextRun===1?'採集殻を展開 · Insight保持中は機体、そうでなければSAFE SITEをtapして帰還':fuel.molecule&&oxidizer.molecule?'COMBUSTION DRIVE · 長押しで継続航行':propellant.molecule?`${formula(propellant.molecule)} BURST · 探索を続けるか持ち帰るか選ぼう`:'通常航行で塵を集め、H₂の材料を持ち帰ろう';
  ...
  function rollbackLaunchTransaction({checkpoint,error}){
    ...
    active=false;paused=false;run=null;returnState=null;shipTap=null;last=0;hudAt=0;messageUntil=0;thermalNotice=null;thermalNoticeUntil=0;
  ...
  function finish(captured=false){
    const expected=captured?'forced-warp-pending':'normal-warp-pending';if(!active||!run||returnState?.phase!==expected)return false;returnState.phase='settling/returning';active=false;cancelAnimationFrame(raf);raf=0;resetInput();shipTap=null;audio.pause();
    ...
    run=null;returnState=null;thermalNotice=null;thermalNoticeUntil=0;onCraft();updateCraft();q('open-supply')?.focus();return true;
  }
  function beginReturn(captured=false){
    if(!active||!run||paused||returnState?.phase!=='exploring')return false;
    if(captured){
      const lossSnapshot={lost:expeditionLoss(run.elementDust,EXPEDITION.captureLoss),insights:runInsightLossSnapshot(run)};
      resetInput();shipTap=null;const duration=renderer.beginForcedReturn(run,lossSnapshot);if(!duration)return false;
      discardRunInsights(run);insightPresentation.clear({showLoss:false});returnState={phase:'forced-return-pending',duration,elapsed:0,lossSnapshot};audio.start();hud();return true;
    }
    const method=returnMethod();if(!method)return false;
    discardActiveInsight(run);insightPresentation.sync(run);resetInput();shipTap=null;run.player.vx=run.player.vy=0;run.player.boost=0;run.player.combustion=false;run.driveHeld=false;renderer.beginReturn(run,'stable');returnState={phase:'normal-extraction-pending',source:method,duration:EXPEDITION.normalExtractionSeconds,elapsed:0};audio.start();audio.event('returnSafe');hud();
    return true;
  }
  function burst(){
    if(!active||paused||returnState?.phase!=='exploring'||run.captured)return;audio.start();
  ...
  function shock(){
    ...
    const slot=run.fuel.shock;if(nearIntactCore)notice('CORE intact · SHOCK CHARGEが必要 · 機体tapまたはSAFE SITEで帰還',3,'○');else if(slot?.molecule&&slot.amount<=0)notice('SHOCK CHARGE EMPTY · 帰還して補給',2,'○');return false;
  }
  function startCombustion(event=null){
    if(!active||paused||returnState?.phase!=='exploring'||run.captured)return;const fuel=run.fuel.fuel,oxidizer=run.fuel.oxidizer,charge=oxidizer.molecule==='oxygen'?combustionChargeFor(fuel.molecule,{fuelAmount:fuel.amount,oxygenAmount:oxidizer.amount,baseSeconds:DRIVES.combustion.packetSeconds}):null;
  ...
  function returnHud(){
    const panel=q('veil-status-panel'),phase=returnState?.phase,method=phase==='exploring'?returnMethod():null,forced=phase==='forced-return-pending',pending=phase?.includes('pending');
    let state='exploring',label='探索中';
    if(forced){state='forced-return';label='DUST EATER · 強制帰還';}
    else if(phase==='normal-extraction-pending'){state='extracting';label='抽出中';}
    else if(phase==='normal-warp-pending'||phase==='forced-warp-pending'||phase==='settling/returning'){state='departing';label='FIELD離脱中';}
    else if(run.overheated){state='overheat';label='OVERHEAT · 推進停止';}
    else if(run.danger==='danger'){state='danger';label='DUST EATER · 危険';}
    else if(method==='insight'){state='insight-ready';label='INSIGHT · 機体をtapして帰還';}
    else if(method==='site'){state='site-ready';label='SAFE SITE · 機体をtapして帰還';}
    else if(run.danger==='warning'){state='warning';label='DUST EATER · 接近';}
    else if(!(run.carriedInsights?.length)&&run.map.safeExtractionSites?.length){state='site-seeking';label='探索中 · SAFE SITEへ';}
    panel.dataset.state=state;panel.dataset.pending=String(pending);panel.textContent=label;panel.setAttribute('aria-label',`FIELD状態: ${label}`);canvas.setAttribute('aria-disabled',String(!method));
  }
  function hud(){
  ...
  function frame(now){
    ...
    syncInsightMarkers();const frameEvents=stepRun(run,input,dt,{consumeCombustion:packet=>resources.consumeCombustion(packet),consumeCoolant:(amount,molecule)=>resources.consumeTank('coolant',molecule,amount)});for(const event of frameEvents){
      ...
      if(event.type==='choDestination'){notice('CHOの最深部に到達 · 機体をtapして記録を持ち帰ろう',8,'◎ ✓ ↩');vibrate(35);}
      ...
      if(event.type==='eaterSpawn'){notice(event.count===1?'DUST EATER · 採集殻の保持場を崩す粒子現象':`DUST EATERS × ${event.count} · 保持場が破綻する前に機体tapまたはSAFE SITEから帰還`,4);vibrate(18);}
      if(event.type==='danger'&&event.level==='warning')notice('保持場への干渉が近い · 距離を作るか帰還',3,'⚠');
      if(event.type==='danger'&&event.level==='danger'){notice('保持場の破綻間近 · 距離を作るか帰還',2,'⚠');vibrate(28);}
      if(event.type==='coreApproach'){notice(event.shockAvailable?'CORE · SHOCK 1 CHARGEでfracture':'CORE intact · SHOCK CHARGEが必要 · 機体tapまたはSAFE SITEで安全に帰還',4,event.shockAvailable?'◉':'○');vibrate(event.shockAvailable?20:10);}
  ...
  const shipHitRadius=48;
  canvas.setAttribute('role','button');canvas.setAttribute('tabindex','0');canvas.setAttribute('aria-label','FIELDの機体。帰還可能な時は機体をtap');
  canvas.addEventListener('pointerdown',event=>{if(event.button!==undefined&&event.button!==0)return;shipTap={id:event.pointerId,x:event.clientX,y:event.clientY};});
  canvas.addEventListener('pointerup',event=>{if(!shipTap||event.pointerId!==shipTap.id)return;const tap=shipTap;shipTap=null;if(Math.hypot(event.clientX-tap.x,event.clientY-tap.y)>14)return;const rect=canvas.getBoundingClientRect(),ship=run?renderer?.screen(run.player.x,run.player.y):null;if(!ship||Math.hypot(event.clientX-rect.left-ship.x,event.clientY-rect.top-ship.y)>shipHitRadius)return;beginReturn(false);});
  for(const type of ['pointercancel','lostpointercapture'])canvas.addEventListener(type,()=>{shipTap=null;});
  ...
  pad.addEventListener('pointerdown',event=>{if(pointer!==null||paused||returnState?.phase!=='exploring')return;event.preventDefault();pointer=event.pointerId;const r=pad.getBoundingClientRect();origin={x:r.left+r.width/2,y:r.top+r.height/2};try{pad.setPointerCapture(pointer);}catch{}audio.start();move(event);});
  ...
  window.addEventListener('keydown',event=>{if(!active||paused||returnState?.phase!=='exploring')return;if(event.key==='Enter'&&!event.repeat&&document.activeElement===canvas){event.preventDefault();beginReturn(false);return;}if(event.key===' '&&event.target.closest?.('button'))return;if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','w','a','s','d',' ','Shift','e','E'].includes(event.key)){event.preventDefault();if(event.key===' '){if(!event.repeat)burst();}else if(event.key==='Shift')startCombustion();else if(event.key==='e'||event.key==='E'){if(!event.repeat)shock();}else keys.add(event.key);audio.start();}});
  ...
  return {get active(){return active;},get run(){return run;},get returnPhase(){return returnState?.phase??null;},get returning(){return returnState?.phase?.startsWith('forced-')?'emergency':returnState?.phase?.startsWith('normal-')&&returnState.phase.endsWith('pending')?'normal-extraction-pending':null;},get lastTelemetry(){return lastTelemetry;},updateCraft,requestExpeditionLaunch,pause,openSupply:(id,use)=>supply.openMolecule(id,use),discovered:id=>supply.discovered(id),usesFor:id=>supply.usesFor(id),tankStatus:(use,id)=>supply.tankStatus(use,id),fillPlan:(use,id)=>supply.fillPlan(use,id),commitFill:(use,id,count)=>supply.commitFill(use,id,count)};
}
