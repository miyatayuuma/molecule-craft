...
export function createVeilUI({resources,canLeave=()=>true,canSupply=canLeave,onBeforeLaunch=()=>true,onCraft=()=>{},onCommit=()=>{}}){
  ...
  let renderer=null,run=null,lastTelemetry=null,active=false,paused=false,raf=0,last=0,hudAt=0,pointer=null,drivePointer=null,origin=null,messageUntil=0,thermalNotice=null,thermalNoticeUntil=0,anchor='continue',anchorLock=null,returnState=null,pendingCraftId=null;
  ...
  function hud(){
    ...
    const thermalState=run.overheated?'overheat':run.coolantActive?'cooling':run.heat>=THERMAL.hotThreshold?'hot':'normal',thermalLabels={normal:'通常',cooling:'自動冷却中',hot:'高温',overheat:'過熱停止'},thermalIndicators={normal:'',cooling:'❄ COOLING',hot:'♨ HOT',overheat:'♨ OVERHEAT'},speedMax=propulsionSpeedMax(run.config),speed=Math.max(0,run.player.speed),speedRatio=speedMax?Math.max(0,Math.min(1,speed/speedMax)):0,activeThermalNotice=thermalNotice&&run.time<thermalNoticeUntil;thermal.hidden=false;thermal.dataset.state=thermalState;thermal.style.background=thermalState==='cooling'?'#0a2828d1':thermalState==='hot'?'#34251ad6':'';q('veil-thermal-state').textContent=thermalState==='overheat'?thermalIndicators.overheat:activeThermalNotice?thermalNotice:thermalIndicators[thermalState];q('veil-heat').textContent='SPEED';thermal.setAttribute('aria-label',`熱状態 ${thermalLabels[thermalState]}。熱 ${Math.round(run.heat)}%。`);if(speedMeter){speedMeter.setAttribute('aria-valuemax',String(speedMax));speedMeter.setAttribute('aria-valuenow',String(speed));}if(speedMeterFill)speedMeterFill.style.transform=`scaleX(${speedRatio})`;
  ...
  function frame(now){
    ...
    for(const event of stepRun(run,input,dt,{consumeCombustion:packet=>resources.consumeCombustion(packet),consumeCoolant:(amount,molecule)=>resources.consumeTank('coolant',molecule,amount)})){
      ...
      if(event.type==='signal'){const excludeIds=new Set(run.carriedInsights);if(run.analysis?.id)excludeIds.add(run.analysis.id);const result=resources.signal(event.region,event.roll,event.choice,{excludeIds});notice(signalText(result),result?.recipe?3:3);if(result?.recipe)offerInsight(result.recipe);supply.update();}
  ...
  q('launch-veil').addEventListener('click',launch);q('veil-return').addEventListener('click',()=>beginReturn(false));q('veil-to-craft').addEventListener('click',()=>{const goal=run?.inspiration?{id:run.inspiration}:growthGoal(resources.state,{cargo:run?.collectedElements??{}}),id=goal.id??null;if(id&&beginReturn(false))pendingCraftId=id;});
