...
import { bindRepeatAction } from './repeat-action.js';
...
export function createVeilUI({resources,canLeave=()=>true,onCraft=()=>{},onStore=()=>false,onCommit=()=>{}}){
  ...
  function updateCraft(){
    ...
    q('make-h2').textContent='H₂ 生成 · 長押し';
  ...
  function hud(){
    ...
    q('veil-fuel').textContent=resources.state.molecules.hydrogen;q('veil-boost').setAttribute('aria-label',`H₂ブースト。${VEIL.boostCost}個消費。キーボードはスペース`);q('veil-boost').classList.toggle('boosting',run.player.boost>0);
    q('veil-boost').setAttribute('aria-disabled',String(!unlocked()||resources.state.molecules.hydrogen<VEIL.boostCost||run.player.cooldown>0));
  ...
  bindRepeatAction(q('make-h2'),{delay:VEIL.holdDelayMs,interval:VEIL.holdIntervalMs,action:()=>{
    if(!canLeave()||!resources.makeHydrogen(1))return false;
    onCommit();const saved=resources.save();updateCraft();
    q('resource-h2').classList.remove('stock-pulse');void q('resource-h2').offsetWidth;q('resource-h2').classList.add('stock-pulse');
    return saved&&resources.state.elements.H>=2;
  }});
