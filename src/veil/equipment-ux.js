...
.mc-flight-compare{position:relative;height:50px;border-radius:10px;background:linear-gradient(90deg,#102735 0 12%,#153344 12% 13%,#0c1c28 13% 100%);overflow:hidden}.mc-flight-compare:after{content:'';position:absolute;left:12%;right:7%;top:50%;height:1px;background:linear-gradient(90deg,#41647555,#41647520)}.mc-run{position:absolute;left:8%;top:50%;width:15px;height:9px;border:1px solid currentColor;border-radius:60% 48% 48% 60%;transform:translateY(-50%);z-index:2}.mc-run:after{content:'';position:absolute;right:100%;top:3px;width:10px;height:2px;background:currentColor;opacity:.55}.mc-run.current{color:#6e8996;opacity:.55;animation:mc-current var(--current-time,1.45s) ease-out infinite}.mc-run.candidate{color:#bceefa;box-shadow:0 0 calc(3px + 10px*var(--candidate-heat)) rgba(238,130,84,var(--candidate-heat));animation:mc-candidate var(--candidate-time,1.2s) ease-out infinite}.mc-run.candidate:after{width:calc(7px + 13px*var(--candidate-drive));opacity:calc(.45 + .5*var(--candidate-drive))}.mc-preview-cells{display:grid;grid-template-columns:1fr 1fr 1fr;gap:5px}.mc-preview-cell{position:relative;height:17px;border:1px solid #294756;border-radius:6px;background:#0b1924;overflow:hidden}.mc-preview-cell:before{position:absolute;inset:0;display:grid;place-items:center;font-size:7px;letter-spacing:.5px;color:#89a9b7;z-index:2}.mc-preview-cell.response:before{content:'MOVE'}.mc-preview-cell.endurance:before{content:'RANGE'}.mc-preview-cell.load:before{content:var(--load-label,'HEAT')}.mc-preview-cell i{position:absolute;inset:auto 0 0;height:100%;transform:scaleX(var(--level));transform-origin:left;background:linear-gradient(90deg,#1b5265,#4fa8bd55);opacity:.5}.mc-preview-cell.load i{background:linear-gradient(90deg,#76412d,#d071454d)}
...
function profileVars(profile){
  ...
  return {
    ...
    drive:String(profile.drive??.5),
    heat:String(profile.heat??0),
  };
}
function renderSelectionPreview(){
  ...
  const loadLabel=role==='fuel'?'HEAT':role==='coolant'?'HOT':role==='propellant'?'BURST':'O₂';
  host.style.setProperty('--candidate-travel',candidateVars.travel);host.style.setProperty('--candidate-time',candidateVars.time);host.style.setProperty('--candidate-drive',candidateVars.drive);host.style.setProperty('--candidate-heat',candidateVars.heat);host.style.setProperty('--current-travel',currentVars.travel);host.style.setProperty('--current-time',currentVars.time);host.style.setProperty('--load-label',`'${loadLabel}'`);
  host.innerHTML=`<div class="mc-preview-head"><strong>${formula}</strong><span class="mc-preview-limit" data-part="${limit?.part??''}">${limit?.text??''}</span></div><div class="mc-flight-compare" aria-hidden="true"><i class="mc-run current"></i><i class="mc-run candidate"></i></div><div class="mc-preview-cells" aria-hidden="true"><span class="mc-preview-cell response"><i style="--level:${candidate.drive}"></i></span><span class="mc-preview-cell endurance"><i style="--level:${candidate.endurance}"></i></span><span class="mc-preview-cell load"><i style="--level:${load}"></i></span></div>`;
