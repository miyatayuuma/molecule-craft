...
function observeStockPreview(){
  ...
  if(!preview||preview._loadoutShortageObserver)return;
...
#supply-dialog .loadout-fuel-label,
#supply-dialog .loadout-oxidizer-label,
#supply-dialog .loadout-coolant-label{left:var(--slot-label-x);top:69%;transform:translateX(-50%)}
...
#supply-dialog .shell-port:before{
  content:'';
  position:absolute;
  inset:0;
  z-index:1;
...
#supply-dialog .shell-port[data-active=true]:before{
  border-color:#a8edf5c4;
  background:radial-gradient(circle at 50% 45%,#a9f1f51a 0 46%,transparent 74%);
  box-shadow:inset 0 0 12px #80d9e42a,0 0 13px #74d6e159;
...
#supply-dialog .shell-port>span:not(.loadout-molecule-pod):not(.tank-scale),
#supply-dialog .shell-port>:scope>small{display:none!important}
...
#supply-dialog .loadout-molecule-pod{
  ...
  padding:3px 4px 2px;
  box-sizing:border-box;
  border:1px solid #7cb9c65c;
  border-radius:14px;
  background:linear-gradient(180deg,#102b3be8,#081923ee);
  box-shadow:inset 0 0 10px #86d9e019,0 3px 10px #0008;
  pointer-events:none;
  user-select:none;
}
...
#supply-dialog .loadout-pod-formula{
  color:#dcecf0;
  font-size:8.5px;
  font-weight:800;
  line-height:1;
...
#supply-dialog .loadout-pod-empty{
  grid-row:1 / -1;
  align-self:center;
  color:#c7e7ed;
  font-size:18px;
...
#supply-dialog .collector-shell-map:has(#shell-coolant[data-active=true]) .loadout-coolant-label{color:#f0fdff;text-shadow:0 1px 4px #000,0 0 9px #83dbe477}
...
#supply-dialog #loadout-stock-preview{
...
#supply-dialog .loadout-shortage-status>i>b{
  ...
  background:linear-gradient(90deg,#6db8c6,#9edce5);
  box-shadow:0 0 8px #77d1dc44;
...
@media(max-width:370px){
  #supply-dialog .collector-shell-map{height:194px!important}
  #supply-dialog #collector-launch-handle{width:70px!important;height:70px!important}
  #supply-dialog .loadout-molecule-pod{width:52px;height:37px;top:calc(100% + 16px)}
  #supply-dialog .loadout-molecule-thumb{width:43px;height:26px}
  #supply-dialog .port-propellant>.loadout-molecule-pod{width:60px;height:40px;top:calc(100% + 15px)}
  #supply-dialog .port-propellant .loadout-molecule-thumb{width:50px;height:29px}
  #supply-dialog .loadout-pod-formula{font-size:8px}
  ...
  #supply-dialog .loadout-fuel-label,#supply-dialog .loadout-oxidizer-label,#supply-dialog .loadout-coolant-label{top:68.5%}
  #supply-dialog .tank-comparison .loadout-stat,#supply-dialog .loadout-charges{grid-template-columns:55px 1fr}
  #supply-dialog .loadout-shortage-status{gap:8px;font-size:9.5px}
}
@media(prefers-reduced-motion:reduce){
  ...
  #supply-dialog .loadout-schematic-lines,
  #supply-dialog .loadout-callout-label,
  #supply-dialog .loadout-shortage-status>i>b{transition:none}
}
`;
