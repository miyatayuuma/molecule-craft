...
export function oxidizerPreviewValues(id,{amount=null}={}){const oxidizer=performanceFor(id,'oxidizer');if(!oxidizer)return null;const available=amount==null?oxidizer.capacity:Math.max(0,amount);return Object.freeze({burnSupply:clamp01(available/Math.max(1,oxidizer.capacity)),oxidizingPower:clamp01(oxidizer.oxidizingPower)});}
...
export function loadoutPreviewValues(use,id,context={}){if(use==='propellant')return propellantPreviewValues(id,{amount:context.amount});if(use==='fuel')return fuelPreviewValues(id,{fuelAmount:context.amount,oxygenAmount:context.oxygenAmount,coolantId:context.coolantId});if(use==='oxidizer')return oxidizerPreviewValues(id,{amount:context.amount});if(use==='coolant')return coolantPreviewValues(id);return null;}
...
function chargeRow(count,{ghost=false,label='回数'}={}){const row=document.createElement('div');row.className='loadout-charges';row.dataset.ghost=String(ghost);const caption=document.createElement('span');caption.textContent=label,dots=document.createElement('div');for(let i=0;i<Math.max(0,count);i++)dots.append(document.createElement('i'));row.append(caption,dots);return row;}
...
function renderOxidizer(host,candidate,current){const stats=document.createElement('div');stats.append(statRow('燃焼量',candidate.burnSupply,current?.burnSupply),statRow('供給力',candidate.oxidizingPower,current?.oxidizingPower));host.append(stats);}
