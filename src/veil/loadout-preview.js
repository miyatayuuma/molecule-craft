...
export function renderLoadoutPreview(host,{use,candidateId,currentId=null,currentAmount=null,oxygenAmount=36,coolantId=null,oxygenTankCapacity=null}={}){host._loadoutPreviewToken?.cancel?.();host._loadoutPreviewToken={cancel(){}};host.replaceChildren();const details=host.closest?.('details');if(details)details.open=true;const context={amount:null,oxygenAmount,coolantId,oxygenTankCapacity},candidate=loadoutPreviewValues(use,candidateId,context),current=currentId?loadoutPreviewValues(use,currentId,{...context,amount:currentAmount}):null;if(!candidate){host.hidden=true;return null;}host.hidden=false;host.dataset.previewKind=use;if(use==='propellant')renderPropellant(host,candidate,current);else if(use==='fuel')renderFuel(host,candidate,current);else if(use==='oxidizer')renderOxidizer(host,candidate);else if(use==='coolant')renderCoolant(host,candidate,current);else if(use==='shock')renderShock(host,candidate,current);return {candidate,current};}
