...
export function createUniverse(seed=1,stock={},{harvestLayout=OXYGEN_HARVEST}={}){
  ...
  for(const [id,label,knots,element]of ROUTES){
    ...
    const route={id,label,element,points,geometry,visual:geometry?{kind:'field-flow',color:'#8fc8d5',fieldLines:geometry.fieldLines,particleSpeed:.18}:null},routeDepletion=map.depletion[element]??0,lanes=activeLaneCount(profile.lanes,routeDepletion),optional=OPTIONAL_ROUTES.has(id);map.routes.push(route);
