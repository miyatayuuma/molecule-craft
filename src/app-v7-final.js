...
function bindUI() {
  document.querySelector('#delete-selected')?.addEventListener('click', () => {
    ...
    atomDamage.set(selectedAtomId, { damage: 0.84, lastHit: now, lastUpdate: now });
    damageAtom(selectedAtomId);
  });
  document.querySelector('#clear-all')?.addEventListener('click', () => {
    markInteraction(); molecule.clear(); placements.clear(); bondDamage.clear(); atomDamage.clear();
    selectedAtomId = null; dragState = null; electronDrag = null; hoveredSnap = null; lastCelebrated = '';
    camera.position.set(5.5,4.3,8.2); controls.target.set(0,0,0); controls.update(); showPivot(controls.target); refresh();
  });
...
function addFreeAtom(symbol) {
  ...
  pulse(`${symbol} を現在の回転中心付近へ配置`); refresh();
}
...
function onPointerDown(e) {
  ...
  if (activePointers.size===2) {
    ...
    twoFingerGesture={startedAt:performance.now(),maxMove:0,midpoint:{x:(pts[0].x+pts[1].x)/2,y:(pts[0].y+pts[1].y)/2}};
    dragState=null; electronDrag=null; hoveredSnap=null; controls.enabled=true; return;
  }
  ...
  if (torsion) {
    dragState={mode:'torsion',atomId,bondA:torsion.a,bondB:torsion.b,movingIds:torsion.movingIds,startX:e.clientX,startY:e.clientY,lastX:e.clientX,lastY:e.clientY,downAt:performance.now(),moved:false};
  ...
  dragState={mode:'translate',atomId,component,startWorld:world.clone(),starts:new Map(component.map(id=>[id,placements.get(id).position.clone()])),startX:e.clientX,startY:e.clientY,downAt:performance.now(),moved:false};
  controls.enabled=false;capture(e);refresh();
}

function onPointerMove(e) {
  const ps=activePointers.get(e.pointerId);
  if (ps) { ps.x=e.clientX;ps.y=e.clientY;const move=Math.hypot(ps.x-ps.startX,ps.y-ps.startY);if(twoFingerGesture)twoFingerGesture.maxMove=Math.max(twoFingerGesture.maxMove,move); }
  markInteraction(); if(activePointers.size>1)return;
  ...
  if(dragState.mode==='torsion'){
    ...
    rotateBranchAroundBond(dragState.bondA,dragState.bondB,dragState.movingIds,THREE.MathUtils.clamp((dx-dy*.35)*.012,-.12,.12));
...
function onPointerUp(e) {
  ...
  if(before>=2){if(twoFingerGesture&&performance.now()-twoFingerGesture.startedAt<330&&twoFingerGesture.maxMove<16){setOrbitPivotFromScreen(twoFingerGesture.midpoint.x,twoFingerGesture.midpoint.y);pulse('2本指タップ位置を回転中心に設定');}if(activePointers.size===0)twoFingerGesture=null;controls.enabled=true;return;}
  ...
  const state=dragState,isTap=!state.moved&&ps&&performance.now()-ps.downAt<330;
  if(isTap)damageAtom(state.atomId);else{const moving=state.mode==='torsion'?state.movingIds:state.component;const candidate=hoveredSnap??findAutoBondCandidate(moving,state.mode==='torsion');if(candidate)formBond(candidate.a,candidate.b,false);if(state.mode==='torsion')settleMolecule(22);}
  dragState=null;hoveredSnap=null;controls.enabled=true;release(e);refresh();
}
function onPointerCancel(e){activePointers.delete(e.pointerId);if(!activePointers.size)twoFingerGesture=null;dragState=null;electronDrag=null;hoveredSnap=null;controls.enabled=true;release(e);refresh();}

function setOrbitPivotFromScreen(clientX,clientY){const rect=renderer.domElement.getBoundingClientRect();pointer.x=((clientX-rect.left)/rect.width)*2-1;pointer.y=-((clientY-rect.top)/rect.height)*2+1;raycaster.setFromCamera(pointer,camera);const hits=raycaster.intersectObjects(moleculeGroup.children,true);let point=hits.length?hits[0].point.clone():null;if(!point){const plane=new THREE.Plane().setFromNormalAndCoplanarPoint(camera.getWorldDirection(new THREE.Vector3()).normalize(),controls.target);point=new THREE.Vector3();if(!raycaster.ray.intersectPlane(plane,point))point.copy(controls.target);}controls.target.copy(point);controls.update();showPivot(point);}
function showPivot(point){disposeGroup(pivotGroup);const ring=new THREE.Mesh(new THREE.TorusGeometry(.18,.018,10,32),new THREE.MeshBasicMaterial({color:0x7dd3fc,transparent:true,opacity:.9,depthTest:false}));ring.position.copy(point);ring.quaternion.copy(camera.quaternion);pivotGroup.add(ring);const right=new THREE.Vector3().setFromMatrixColumn(camera.matrixWorld,0).normalize().multiplyScalar(.27),up=new THREE.Vector3().setFromMatrixColumn(camera.matrixWorld,1).normalize().multiplyScalar(.27);const h1=cylinderBetween(point.clone().sub(right),point.clone().add(right),.008,0x7dd3fc,.8),h2=cylinderBetween(point.clone().sub(up),point.clone().add(up),.008,0x7dd3fc,.8);h1.material.depthTest=false;h2.material.depthTest=false;pivotGroup.add(h1,h2);pivotVisibleUntil=performance.now()+900;}

function chooseTorsionForAtom(atomId){const candidates=[];for(const bond of molecule.bonds){if(!isRotatableBond(bond))continue;const sides=cutSides(bond.a,bond.b);if(!sides)continue;const inA=sides.aSide.has(atomId),inB=sides.bSide.has(atomId);if(!inA&&!inB)continue;const movingIds=[...(inA?sides.aSide:sides.bSide)];if(!movingIds.includes(atomId)||movingIds.every(id=>atomById(id)?.element==='H'))continue;candidates.push({a:bond.a,b:bond.b,movingIds,size:movingIds.length});}candidates.sort((a,b)=>a.size-b.size);return candidates[0]??null;}
...
function damageAtom(id){const atom=atomById(id);if(!atom)return;const now=performance.now(),required=atomBreakTaps(atom.element),prev=atomDamage.get(id),base=prev?currentDamage(prev,now):0,damage=Math.min(1,base+1/required);atomDamage.set(id,{damage,lastHit:now,lastUpdate:now,required});selectedAtomId=id;if(damage<.999){pulse(`${atom.element} が崩れています ${Math.round(damage*100)}%`);moleculeGroup.userData.shakeUntil=now+110;if(navigator.vibrate)navigator.vibrate(7);refresh();return;}atomDamage.delete(id);const pos=placements.get(id)?.position.clone()??new THREE.Vector3(),color=ELEMENTS[atom.element].color;spawnMist(pos,color);molecule.removeAtom(id);placements.delete(id);selectedAtomId=null;lastCelebrated='';settleMolecule(30);pulse(`${atom.element} が霧散しました`);if(navigator.vibrate)navigator.vibrate([12,18,35]);refresh();}
