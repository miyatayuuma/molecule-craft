...
function findNearestCompatibleElectron(clientX,clientY,sourceAtomId,sourceIndex){
  let best=null;for(const ev of electronVisuals){if(ev.atomId===sourceAtomId)continue;if(!canPairAtoms(sourceAtomId,ev.atomId))continue;const screen=worldToScreen(ev.visible.position),d=Math.hypot(clientX-screen.x,clientY-screen.y);if(!best||d<best.distance)best={atomId:ev.atomId,index:ev.index,distance:d,screenX:screen.x,screenY:screen.y};}return best;
}
