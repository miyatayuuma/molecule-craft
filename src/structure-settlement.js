...
export function createStructureSettlement({THREE,molecule,placements,ids,lockedIds=new Set(),bondLengthFor,geometryFor,radiusFor,nonbondedDistanceFor,now=0,duration=240}) {
  ...
  const score=e=>e.finite&&!e.topologyLimited?Math.max(e.bondRelative/.025,e.angleRadians/(8*Math.PI/180),e.planeDistance/.035,(e.overlapRelative??0)/.15):Infinity;
  let errors=solver.measureError(),bestErrors=errors,bestScore=score(errors),best=copy();
  ...
  return {
    ...
    advance(time,{clock=()=>performance.now(),budgetMs=5}={}){
      if(done)return {done:true,converged:bestScore<=1,errors:bestErrors,steps};
      ...
      if(blendAt===null){
        ...
        while(count<64&&steps<768&&clock()-started<budgetMs&&bestScore>1&&errors.finite&&!errors.topologyLimited){
          ...
          if(count%8===0){errors=solver.measureError();const next=score(errors);if(next<bestScore){bestScore=next;best=copy();bestErrors=errors;}}
        }
        errors=solver.measureError();const next=score(errors);if(next<bestScore){bestScore=next;best=copy();bestErrors=errors;}
      ...
      return {done,converged:done&&bestScore<=1,errors:bestErrors,steps};
    },
  };
}
