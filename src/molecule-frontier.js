...
import {isFieldProgressionReserved} from './veil/progression-reserved-molecules.js';
...
export function getFrontierCandidates(graph,{discoveredIds=[],knownRecipeIds=[]}={}){if(!validGraph(graph))return Object.freeze([]);const discovered=new Set(sorted(discoveredIds).filter(id=>graph.nodeById(id))),known=ids(knownRecipeIds),found=new Set();for(const id of [...discovered].sort((a,b)=>a.localeCompare(b)))for(const next of neighbors(graph,id))if(!discovered.has(next)&&!known.has(next)&&!isFieldProgressionReserved(next))found.add(next);return Object.freeze([...found].sort((a,b)=>a.localeCompare(b)).map(id=>metadata(graph,id,discovered)).filter(Boolean));}
